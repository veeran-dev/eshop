<?php
class AdminSampleControllerCore extends AdminController
{
    ...
    public function __construct()
    {
		...
		CONCAT(c.`firstname`) AS `customer`,CASE WHEN c.id_buyer=3 THEN "Yes" ELSE "No" END as id_buyer,
		osl.`name` AS `osname`,
        state.name as address_state,
		os.`color`,fcentre.city_name as city, 
        ...
        LEFT JOIN `'._DB_PREFIX_.'fulfillment_centre` fcentre ON(a.`id_fc`=fcentre.`id_fulfillment_centre`)
        ...
        foreach ($statedata AS $statevalue)
            $state_array[$statevalue['id_state']] = $statevalue['name'];
        ...
        $fcArray=array();
        $fcOrders=FulfillmentCentre::getAllFCentres();
        foreach($fcOrders as $fco)            
            $fcArray[$fco['id_fulfillment_centre']]=$fco['city_name'];
        
        //$buyerArray=array(9=>'No',3=>'Yes');
        $buyerArray=array(3=>'Yes',0=>'No');
        ...
        $this->fields_list = array(
           ...
           'city' => array(
                ...
                'type' => 'select',
                'list' => $fcArray,
                'filter_key' => 'fcentre!id_fulfillment_centre',
                'filter_type' => 'int',
            ),
           /*'address_state' => array(
                'title' => $this->l('State'),
    ...
    public function postProcess()
    {
            ...
            if ($this->tabAccess['edit'] === '1') {
                ...
                if (!Validate::isLoadedObject($order_carrier)) {
                    ...
                    if ($order_carrier->update()) {
                        ...
                        else if (@Mail::Send((int)$order->id_lang, 'perks_in_transit', Mail::l('Package in transit', (int)$order->id_lang), $templateVars,
        ...
        elseif (Tools::isSubmit('submitState') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                ...
                if (!Validate::isLoadedObject($order_state)) {
                    ...
                    $newOrderStatusId = (int)(Tools::getValue('id_order_state'));
                    ...
                    if(($newOrderStatusId == Configuration::get('PS_OS_INVOICE_GEN')) OR ($newOrderStatusId == Configuration::get('PS_OS_DELIVERED')))
                    {
                        ...
                        if(!$order->checkIfAllProductsDelivered()){
                            ...
                            $this->errors[] = Tools::displayError('All products are not delivered yet.');
                            return;
                        }
                    }

					if($newOrderStatusId == OrderState::FLAG_PENDING_PAYMENT && !$order->isInvoiceSubmitted())
                    {
                        $this->errors[] = Tools::displayError('Invoice not submitted yet.');
                        return;
                    }

                    if($newOrderStatusId == OrderState::FLAG_INVOICE_SUBMITTED && !$order->isInvoiceGenerated())
                    {
                        $this->errors[] = Tools::displayError('Invoice not submitted yet.');
                        return;
                    }
                    ...
                    if($newOrderStatusId == OrderState::FLAG_INVOICE_SUBMITTED)
                    {
						...
						if($delivery_count != 0 )
						{
							$this->errors[] = Tools::displayError('DR Upload Pending.');
							return;
						}
                    }
                    ...
                    if ($current_order_state->id != $order_state->id) {
                        ...
                        if($newOrderStatusId == OrderState::FLAG_PENDING_PAYMENT)
                        {
                            ...
                            if($status == 0)
                            {
                                $this->errors[] = Tools::displayError('Invoice submitted status not found');
                            }
                        }
        ...
        elseif(Tools::isSubmit('updateLabel'))
        {
            $kob_box = Tools::getValue('kob_box');
            $other_box = Tools::getValue('other_box');
            $id_delivery = Tools::getValue('id_delivery');

            Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'delivery` SET `kob_boxes` = '.$kob_box.', `other_boxes` = '.$other_box.' WHERE `id_delivery` = '.(int)($id_delivery).'');
        }
        elseif (Tools::isSubmit('deliveryTimeButton') AND ($id_order = (int)(Tools::getValue('id_order'))) AND Validate::isLoadedObject($order = new Order($id_order)))
        {
            ...
            if ($this->tabAccess['edit'] === '1')
            {
                ...
                $new_estimated_delivery_time = pSQL(Tools::getValue('deliverytime'));
                $old_estimated_delivery_time = $order->estimated_delivery_time;
                $order->estimated_delivery_time = $new_estimated_delivery_time;
                $customer = new Customer($order->id_customer);
                $customer_name = $customer->firstname;
                if($order->update()){
                    //Send email alert
                    $employee = new Employee((int)($context->cookie->id_employee));

                    $message =$employee->firstname.' changed Estimated Delivery Time from '.$old_estimated_delivery_time.' to '.$new_estimated_delivery_time.'';
                    $messageObject = new Message();
                    $messageObject->id_employee = $context->cookie->id_employee;
                    ...
                    Tools::redirectAdmin($currentIndex.'index.php?controller=AdminOrders&id_order='.$id_order.'&token='.$this->token);
                }
                else
                    $this->_errors[] = Tools::displayError('Invalid Date Time Format.');
            }
            else
                $this->_errors[] = Tools::displayError('You do not have permission to edit here.');
        }
        elseif (Tools::isSubmit('uploadDRScan') AND ($id_order = (int)(Tools::getValue('id_order'))) AND Validate::isLoadedObject($order = new Order($id_order)))
        {
            $dr_name = Tools::getValue('dr_number');
            $id_delivery = (int)Tools::getValue('id_delivery');
            $delivery = new Delivery($id_delivery);

            if ($_FILES["file"]["error"] > 0) {
              //echo "Error: " . $_FILES["file"]["error"] . "<br>";
              $this->_errors[] = Tools::displayError('Error in file upload.');
            } 
            elseif($_FILES["file"]["size"] > 2098576)
            {
                $this->_errors[] = Tools::displayError('File too Big. Please upload file less than 2 mb.');
            }
            else {
              ...
              if(move_uploaded_file($_FILES["file"]["tmp_name"],""._PS_CORE_DIR_."/scanedDRs/".$dr_name . "." . $extension)){
                    
                    $delivery->dr_file_name = $dr_name . "." . $extension;
                    $delivery->update();
        ...
        elseif(Tools::isSubmit('submitfc'))
        {
            $context = Context::getContext();        
            $fc_id = (int)(Tools::getValue('fcentre'));
            $order = new Order((int)(Tools::getValue('id_order')));

            $old_fc = $order->id_fc;

            if($fc_id == $order->id_fc || $fc_id == 0)
                return false;
            $order->id_fc = $fc_id;
            ...
            if($order->update())
            {
                ...
                Mail::Send((int)$context->cookie->id_lang, 'fcChange', Mail::l($em_sub, (int)$context->cookie->id_lang), $data,  'scn.operations@kobster.com', 'Supply Chain Team', NULL, NULL, NULL);
            }
        }
        ...
        elseif (Tools::isSubmit('submitMessage') && isset($order)) 
        {
            if ($this->tabAccess['edit'] === '1') 
            {
                ...
                else 
                {
                    ...
                    {
                        $messageObject = new Message();
                        $messageObject->id_employee = (int)Context::getContext()->cookie->id_employee;
                        $messageObject->message = htmlentities(Tools::getValue('message'), ENT_COMPAT, 'UTF-8');
                        $messageObject->id_order = (int)Tools::getValue('id_order');
                        $messageObject->private = Tools::getValue('visibility');

                        if (!$messageObject->add()) 
                        {
                            $this->errors[] = Tools::displayError('An error occurred while saving the message.');
                        } 
                        elseif ($messageObject->private) 
                        {
                            ...
                            $customer = new Customer($order->id_customer);
                            $group = new Group($customer->id_default_group, (int)Context::getContext()->cookie->id_lang);
                            ...
                            if($customer->id_relationship_manager != "" && $customer->id_relationship_manager != 0)
                            {
                                @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                    Mail::l('Order Comment - #'. $messageObject->id_order .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, $relationship_manager->email,
                                    'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                            }
                            else
                            {
                                @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                    Mail::l('Order Comment - #'. $messageObject->id_order .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, 'support@kobster.com',
                                    'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                            ...
                            {
                                @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                    Mail::l('Order Comment - #'. $messageObject->id_order .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, $fc_head->email,
                                    'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                            }

							@Mail::Send((int)($order->id_lang), 'order_internal_comment',
									Mail::l('Order Comment - #'. $messageObject->id_order .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, 'ops@kobster.com',
									'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
							@Mail::Send((int)($order->id_lang), 'order_internal_comment',
									Mail::l('Order Comment - #'. $messageObject->id_order .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, 'lingakani.g@kobster.com',
									'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                        ...
                        else 
                        {
                            ...
                            if (Validate::isLoadedObject($order))
                            {
                                $varsTpl = array('{lastname}' => $customer->lastname, '{firstname}' => $customer->firstname, '{id_order}' => $messageObject->id_order, '{message}' => (Configuration::get('PS_MAIL_TYPE') == 2 ? $messageObject->message : nl2br2($messageObject->message)));
    ...
    public function renderView()
    {
        ...
        $delivery_slip=$this->getDr($order->id);
        $fcOrders=FulfillmentCentre::getAllFCentres();
    ...
    public function getDr($order_id)
    {
        $query='SELECT kd.* FROM `kob_delivery_details` dd
                    LEFT JOIN `kob_delivery` kd ON(kd.`id_delivery`=dd.`id_delivery`)
                    WHERE dd.`id_order`='.$order_id.' GROUP BY id_delivery';
