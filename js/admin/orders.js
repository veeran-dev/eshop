...
$(document).ready(function() {
	...
	$('img.js-disabled-action').css({"opacity":0.5});
	 ...
	 $(".quantity-checkbox").change(function(){
	 	...
	 	setCancelQuantity(this, id_order_detail);
	 });
	 $("#generate_dr").click(function(){
	 	$('#delivery-box-block').slideToggle();
	 });
	 $("#generateDR").bind("click", generateDR);

	 //Function to generate partial DR
function generateDR()
{
	var kob_box = $('#kob-box').val();
	var other_box = $('#other-box').val();
	var url      = window.location.href;

	if(kob_box == "" || isNaN(kob_box))
		alert('Please enter number of kobster boxes');
	else if(other_box == '' || isNaN(other_box))
		alert('Please enter number of other boxes');
	else if(kob_box != '' && other_box != '')
	{
		$.ajax({
			...
			datatype: 'json',
			url:'generate-dr.php',
			data: $('#productListOrder').serialize(),
			cache: false,
	...
	function setCancelQuantity(itself, id_order_detail)
	{
		$('#drQuantity_' + id_order_detail).val($(itself).attr('checked') ? $('#updateQuantity_'+id_order_detail).html() : '');
	}	
});

function setCancelQuantity(itself, id_order_detail)
{
	console.log("setCancelQuantity");
	$('#drQuantity_' + id_order_detail).val($(itself).attr('checked') ? $('#updateQuantity_'+id_order_detail).html() : '');
}
