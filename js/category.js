...
function reloadContent(params_plus)
{
	...
	if(ajaxLoaderOn == 0){
		$('#product_list_div').prepend($('#layered_ajax_loader').html());
		$('#product_list_div').css('opacity', '0.5');
		ajaxLoaderOn = 1;
	}
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			if (result.productList)
				$('#product_list_div').replaceWith(utf8_decode(result.productList));
			else
				$('#product_list_div').html('');

			$('#product_list_div').css('opacity', '1');
			if ($.browser.msie) // Fix bug with IE8 and aliasing
				$('#product_list_div').css('filter', '');
			...
			if (document.getElementById('categoryImage') == null){
			document.getElementById('layered_block_left').style.top = "-74px";
			document.getElementById('layered_block_left').style.visibility = "visible";
			}
			else{
				document.getElementById('layered_block_left').style.visibility = "visible";
			}
			...
			$(".quickView_left img.lazyImage").jail({
				triggerElement:'a.quickViewButton',
				event: 'click',
				effect: 'fadeIn',
				speed : 500
			});	
...
function updateProductUrl()
{
	...
	if (typeof(param_product_url) != 'undefined') {
		$.each($('ul#product_list_div li.ajax_block_product .product_img_link,'+
				'ul#product_list_div li.ajax_block_product h3 a,'+
				'ul#product_list_div li.ajax_block_product .product_desc a,'+
				'ul#product_list_div li.ajax_block_product .lnk_view'), function() {
