...
class sisterCategory extends Module
{
 	private $_html;

	public function __construct()
 	{
 	 	$this->name = 'sisterCategory';
 	 	$this->version = '1.3';
		...
		$this->displayName = $this->l('Sister Products');
		$this->description = $this->l('Display Sister products on the product page.');
	...
	public function hookProductFooter($params)
	{
        ...
        $parent_category = new Category($category->id_parent, (int)($cookie->id_lang));
		
		if($category->id_parent > 1)
		{
			// Get parent category's subcategories (which is current category's siblings, including it self).
			$category_siblings = $parent_category->getSubCategories((int)($cookie->id_lang));
			
			$categoryProducts[] = array();
			$categoryTemp[] = array();
			foreach($category_siblings as $key => $category_sibling)
			{
				if($category_sibling['id_category'] <> $category->id_category)
				{
					$category_sib = New Category((int)($category_sibling['id_category']));
					$categoryTemp = $category_sib->getProducts((int)($cookie->id_lang), 1, 5, NULL, NULL, false, true, true, 5, true);
					foreach ($categoryTemp AS $categoryTempP)
					{
						array_push($categoryProducts, $categoryTempP);
					}
				}
			}
			
			// Get infos
			//$categoryProducts = $category->getProducts((int)($cookie->id_lang), 1, 7, NULL, NULL, false, true, true, 6, true); /* 100 products max. */
			$sizeOfCategoryProducts = (int)sizeof($categoryProducts);
			if($sizeOfCategoryProducts > 6)
			{
				$categoryProductsFiltered[] = array();
				shuffle($categoryProducts);
				$categoryProductsFiltered = array_slice($categoryProducts, 0, 7);
				$categoryProducts[] = array();
				$categoryProducts = $categoryProductsFiltered;
				$sizeOfCategoryProducts= 6;
			}
			...
			if (is_array($categoryProducts) AND sizeof($categoryProducts))
			{
				foreach ($categoryProducts AS $key => $categoryProduct)
					if ($categoryProduct['id_product'] == $idProduct OR $categoryProduct['id_product'] == 0)
					{
						unset($categoryProducts[$key]);
						break;
					}
				
				if((int)sizeof($categoryProducts) > 6)
				{
					foreach ($categoryProducts AS $key => $categoryProduct)
					{
						unset($categoryProducts[$key]);
		...
		return $this->display(__FILE__, 'sisterCategory.tpl');
	}
	
	public function hookHeader($params)
	{
		/*Tools::addCSS($this->_path.'sisterCategory.css', 'all');
		Tools::addJS(array($this->_path.'sisterCategory.js', _PS_JS_DIR_.'jquery/jquery.serialScroll-1.2.2-min.js'));*/
	}
}
