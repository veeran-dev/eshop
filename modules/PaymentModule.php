...
abstract class PaymentModuleCore extends Module
{
	...
	*/
	public function validateOrder($id_cart, $id_order_state, $amountPaid, $paymentMethod = 'Unknown', $message = NULL, $extraVars = array(), $currency_special = NULL, $dont_touch_amount = false, $secure_key = false, $dash=0)
	{
 		global $cart;
		...
		//if dash payment is not work means just hide this 
		/*if (!$this->active)
		...
		if (Validate::isLoadedObject($cart) AND $cart->OrderExists() == false)
		{
			...
			$id_fc=$fc->getFcByAddressId((int)($cart->id_address_delivery));
			$order = new Order();
			...
			if ($result AND isset($order->id))
			{
				...
				if($customer->id_buyer == 3 AND (int)$cookie->role != 1)
				{
				...
				if ($id_order_state != Configuration::get('PS_OS_ERROR') AND $id_order_state != Configuration::get('PS_OS_CANCELED') AND $customer->id)
				{
					...
					if($cookie->creator_customer_id != 0)
					{
						$child = new Customer((int)$cookie->creator_customer_id);
						$child_name = $child->firstname;
 						$approver_data_name =  $customer->firstname;
						$creator_old_order_id = $cookie->creator_old_order_id;
					}
					else{
					...
					if (Validate::isEmail($customer->email))
						...
						$sendnotification->notify(1, $data,$order->id,$fileAttachment);*/

						if((int)$cookie->role != 1)
						{
							$em_sub = 'Order confirmation | Your Order with Kobster.com [#'. $order->id .'] has  been successfully placed!';
							Mail::Send((int)$order->id_lang, 'order_conf', Mail::l($em_sub, (int)$order->id_lang), $data, $customer->email, $customer->firstname.' '.$customer->lastname, NULL, NULL, $fileAttachment);
						}
						else
						{
							$em_sub = 'Order confirmation | The Order with Kobster.com [#'. $order->id .'] is pending for approval';
							//Send email stating pending approval
							Mail::Send((int)$order->id_lang, 'order_pending_approval', Mail::l($em_sub, (int)$order->id_lang), $data, $customer->email, $customer->firstname.' '.$customer->lastname, NULL, NULL, $fileAttachment);
							
							//Send email to approver
							$approver = new Customer($customer->getApprover());
							
							Mail::Send((int)$order->id_lang, 'order_submit_approval', Mail::l($em_sub, (int)$order->id_lang), $data, $approver->email, $approver->firstname.' '.$approver->lastname, NULL, NULL, $fileAttachment);
							
							//SMS Approver
							if($approver->mobile)
							{
								$msgtxt='Hello '.$approver->firstname.' , a new order # '.$order->id.' is pending for your approval. - Kobster.com';
								SMSAlert::sendSMSAlert($approver->mobile, $msgtxt);
							}
						}
						
						if($invoice->phone_mobile)
						{
							$msgtxt='Hello '.$customer->firstname.' '.$customer->lastname.' , Your order # '.$order->id.' has been placed successfully. - Kobster.com';
						...
						if($cookie->creator_customer_id != 0)
						{
							...
							$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('UPDATE '._DB_PREFIX_.'orders set id_customer = '.$cookie->creator_customer_id.' where id_order = '.$order->id.'');
							
							/*Change loyalty point to 0 for cancelled order*/
							 Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('UPDATE '._DB_PREFIX_.'loyalty set points = 0 where id_order = '.$cookie->creator_old_order_id.'');
							 
							 /*Change loyalty id_customer to child_id_customer for newly placed order*/
							 Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('UPDATE '._DB_PREFIX_.'loyalty set id_customer = '.$cookie->creator_customer_id.' where id_order = '.$order->id.'');
							...
							Mail::Send((int)$order->id_lang, 'revise_order_conf', Mail::l($em_sub, (int)$order->id_lang), $data, $child->email, $child->firstname.' '.$child->lastname, NULL, NULL, $fileAttachment);
 							if($result == 1)//Reset the cookie so that next time it will check where approver  or child order.
							{
								$cookie->creator_customer_id=0;	
								$cookie->creator_old_order_id=0;
							}
						}
				}
