...
{if $status == 'ok'}
	<p>{l s='Your order on' mod='bankwire'} <span class="bold">{$shop_name}</span> {l s='is complete.' mod='bankwire'}
		<br /><br />
		{l s='Please perform a NEFT/RGTS/Direct Deposit to the following account:' mod='bankwire'}
		...
		<br /><br />{l s='With these details:' mod='bankwire'}<br /> <br /><span class="bold">{if $bankwireDetails}{$bankwireDetails}{else}___________{/if}</span>
		<br /><br />{l s='To this bank:' mod='bankwire'}<br /> <br /><span class="bold">{if $bankwireAddress}{$bankwireAddress}{else}___________{/if}</span>
		<br /><br />{l s='Do not forget to insert your order #' mod='bankwire'} <span class="bold">{$id_order}</span> {l s='in the subject of your Online Transfer' mod='bankwire'}
