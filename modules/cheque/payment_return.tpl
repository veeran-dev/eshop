...
{if $status == 'ok'}
	<p>{l s='Your order on' mod='cheque'} <span class="bold">{$shop_name}</span> {l s='is complete.' mod='cheque'}
		...
		<br /><br />{l s='In favour of:' mod='cheque'} <span class="bold">{if $chequeName}{$chequeName}{else}___________{/if}</span>
		<br /><br />{l s='Payable at:' mod='cheque'} <span class="bold">{if $chequeName}Chennai{else}___________{/if}</span>
		<br /><br />{l s='Send it to the following address:' mod='cheque'}<br /><br /><span class="bold">{if $chequeAddress}{$chequeAddress}{else}___________{/if}</span>
