...
if ($cookie->isLogged())
{
	...
	$res_wishlist= WishList::gettotalproByIdCustomer($cookie->id_customer);
	$wishlists = WishList::getByIdCustomer((int)($cookie->id_customer));
	
	/*$wish_products = WishList::getProductOnlyByIdCustomer($cookie->id_customer, $cookie->id_lang);
	
	$wish_products = Product::getProductsProperties((int)$cookie->id_lang, $wish_products);
	for ($j = 0; $j < sizeof($wish_products); $j++)
	{
		$total_wish_price += ($wish_products[$j]['price_tax_exc']) * ((int) $wish_products[$j]['wishlist_quantity']);
		$total_wish_qty += (int) $wish_products[$j]['wishlist_quantity'];
	}*/
	
	
	$smarty->assign('total_wish_price', $res_wishlist[0]['totalprice']);
	$smarty->assign('total_wish_qty', $res_wishlist[0]['totalqty']);
