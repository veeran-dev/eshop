	...
	if(!$('.cop_profile_value'+id).hasClass("border"))
	{
		...
		$.ajax({
			...
			success: function(jsonData) 
			{
				if(jsonData == 0) 
				{
					toast("Please select delivery address to proceed","error");
				}
				else 
				{
					alias=$('#alias'+id).val();
...
$(document).ready(function() {
	$('#uploadFile').live('change',function() {
		...
		if(po_file_elite.size > 2097152){
			...
			$('#labelPo').html(po_file_elite.name);
		}
	});
 	...
 	$(function(){
		...
		$.ajax({
			...
			{	
				...
				{
					if(jsonData[i].name != 'EBS' && jsonData[i].name != 'EPAYLATER') {
						$("#payment_options").append("<div id='payment_options"+i+"' class='payment-option-margin' ><a  class='' onclick='paymentMode("+ jsonData[i].id_payment +");'><img src='dash/img/payment-methods/"+ jsonData[i].id_payment +".svg'/><br/>"+jsonData[i].name+"</a></div>");
					}
				}
			}	
		});
...
function toRemoveCartProduct(id_product,id_combination,id_customization,id) {
	//$("#"+id).css("display","none");
 		$.ajax({
			...
			cache: false,
			dataType : "json",
			data: 'delete=1&id_product=' + id_product + '&ipa=' + ((id_combination != null && parseInt(id_combination)) ? id_combination : 0) + ((id_customization && id_customization != null) ? '&id_customization=' + id_customization : '') + '&token=' + static_token + '&ajax=true',
			success: function(jsonData)	{				
				if(rm_portal !=1)
...
function paymentMode(paymentmode)
{
	...
	var po_file = document.getElementById('uploadFile');

	if((po_number == "" || po_file.value == "") && po_mandatory == 1)
	{
		alert("PO number or file is missing. Please upload to complete purchase.","error");
		return false;
	}
	else if(po_number == "" && po_file.value != "") {
		alert("PO number or file is missing. Please upload to complete purchase.","error");
		return false;
	}

	if (po_number != "" && !(/\S/.test(po_number))) {
		alert("Please enter valid PO number.", "error");
		return false;
	}
