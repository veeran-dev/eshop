...
<!-- ADDRESSES AND CUSTOMER INFORMATIONS -->
<table style="width: 100%">
	<tr>
		<td style="width: 20%; padding-right: 7px; vertical-align: top; font-size: 7pt;">
			<!-- CUSTOMER INFORMATIONS -->
			<span style="font-size: 10pt; color: #9E9F9E">{l s='Order Summary' pdf='true'}</span><br />
			{l s='ORDER:' pdf='true'}
			#{$order_id}<br />
			...
			{dateFormat date=$order->invoice_date full=0}<br />
...
<!-- PRODUCTS TAB -->
			...
			<table style="font-size: 8pt;">
				<tr style="line-height:4px;">
					<td style="text-align: left; background-color: #9E9F9E; color: #FFF; padding-left: 10px; font-weight: bold; width: 5%">S.No</td>
					{$product_reference_width = 25}
					{if Configuration::get('PS_PDF_IMG_INVOICE')}
						{$product_reference_width = $product_reference_width - 10}
						<td style="text-align: left; background-color: #9E9F9E; color: #FFF; padding-left: 10px; font-weight: bold; width: 10%">{l s='Image' pdf='true'}</td>
					{/if}
					{if !$tax_excluded_display}
						{$product_reference_width = $product_reference_width - 10}
					{/if}
					<td style="text-align: left; background-color: #9E9F9E; color: #FFF; padding-left: 10px; font-weight: bold; width: {$product_reference_width}%">{l s='Description' pdf='true'}</td>
					<td style="text-align: left; background-color: #9E9F9E; color: #FFF; padding-left: 10px; font-weight: bold; width: 20%">{l s='Reference' pdf='true'}</td>
					<!-- unit price tax excluded is mandatory -->
					{if !$tax_excluded_display}
						<td style="background-color: #9E9F9E; color: #FFF; text-align: right; font-weight: bold; width: 10%">{l s='Unit Price' pdf='true'} <br />{l s='(Tax Excl.)' pdf='true'}</td>
					{/if}
					<td style="background-color: #9E9F9E; color: #FFF; text-align: right; font-weight: bold; width: 10%">{l s='Unit Price' pdf='true'}</td>
					<td style="background-color: #9E9F9E; color: #FFF; text-align: right; font-weight: bold; width: 10%">{l s='Discount' pdf='true'}</td>
					<td style="background-color: #9E9F9E; color: #FFF; text-align: center; font-weight: bold; width: 10%">{l s='Qty' pdf='true'}</td>
					<td style="background-color: #9E9F9E; color: #FFF; text-align: right; font-weight: bold; width: 15%">{l s='Total' pdf='true'}</td>
				</tr>
				...
				<tr style="line-height:6px;background-color:{$bgcolor};" {if Configuration::get('PS_PDF_IMG_INVOICE') && isset($order_detail.image) && $order_detail.image->id && isset($order_detail.image_size)}height="{$order_detail['image_size'][1]}"{/if}>
					<td style="text-align: left;">{$order_detail.i}</td>
					{if Configuration::get('PS_PDF_IMG_INVOICE')}
						<td style="text-align: left;">{if isset($order_detail.image) && $order_detail.image->id}{$order_detail.image_tag}{/if}</td>
					{/if}
					<td style="text-align: left; width: {$product_reference_width}%">{$order_detail.product_name}</td>
					<td style="text-align: left; width: {$product_reference_width}%">{if isset($order_detail.product_reference) && !empty($order_detail.product_reference)}{$order_detail.product_reference}){/if}</td>
					<!-- unit price tax excluded is mandatory -->
					{if !$tax_excluded_display}
