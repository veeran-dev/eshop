...
<div id="cart_title" class="page-heading">{l s='My Cart'}{if !isset($empty) && !$PS_CATALOG_MODE}<span id="summary_products_quantity">&nbsp;({$productNumber}) </span>{/if}
...
{if $perksVoucherCode != ''}
	<div class="professional-id-success">Thanks for uploading your ID proof,  grab your voucher code: <span class="voucher-code">{$perksVoucherCode}</span> worth of <strong>Rs.100.</strong></div>
{/if}
...
{if isset($empty)}
	<div class="empty-cart-state">
		<img src="{$img_dir}empty-cart.png" alt="empty-shopping-cart">
		{* <p class="alert alert-warning">{l s='Your shopping cart is empty.'}</p> *}
		<a class="button-black center-block" href="{$base_dir}{if $isPerks==1}perksDeals{else}shop{/if}">Start Shopping</a>
	</div>
...
{else}
	...
	<div id="order-detail-content" class="table_block table-responsive">
        ...
        <table id="cart_summary" class="table  {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<thead>
				<tr>
					<th class="cart_product first_item font-color-gray">{l s='Product'}</th>
					<th class="cart_description item font-color-gray">{l s=''}</th>
			...
			<tfoot>
				...
				{if $use_taxes}
					{if $priceDisplay}
						<tr class="cart_total_price">
							<td colspan="3" class="tfoot-label">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products '}{/if}</td>
					...
					{else}
						<tr class="cart_total_price">
							...
							<td colspan="3" class="tfoot-label">{if $display_tax_label}{l s='Total products  (tax incl.)'}{else}{l s='Total products'}{/if}</td>
				...
				{else}
					<tr class="cart_total_price">
						...
						<td colspan="3" class="tfoot-label">{l s='Total products '}</td>
						<td colspan="2" class="price" id="total_product">{displayPrice price=$total_products}</td>
					</tr>
				{/if}
				...
				{if $total_shipping_tax_exc <= 0 && (!isset($isVirtualCart) || !$isVirtualCart) && $free_ship}
					<tr class="cart_total_delivery{if !$opc && (!isset($cart->id_address_delivery) || !$cart->id_address_delivery)} unvisible{/if}">
						...
						<td colspan="3" class="tfoot-label no-border">{l s='Total shipping'}</td>
					...
					{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
						{if $priceDisplay}
							<tr class="cart_total_delivery{if $total_shipping_tax_exc <= 0} unvisible{/if}">
								<td colspan="3" class="tfoot-label no-border">{if $display_tax_label}{l s='Total shipping (tax excl.)'}{else}{l s='Total shipping'}{/if}</td>
						...
						{else}
							<tr class="cart_total_delivery{if $total_shipping <= 0} unvisible{/if}">
								<td colspan="3" class="tfoot-label no-border">{if $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}</td>
					...
					<td colspan="3" class="tfoot-label no-border">
						{if $display_tax_label}
							{if $use_taxes && $priceDisplay == 0}
								{l s='Discount (tax incl.)'}
							{else}
								{l s='Discount (tax excl.)'}
							{/if}
						{else}
							{l s='Discount'}
						{/if}
				...
				{if $use_taxes && $show_taxes && $total_tax != 0 }
					{if $priceDisplay != 0}
					<!--<tr class="cart_total_price">
						<td colspan="{$col_span_subtotal}" class="text-right no-border-top">{if $display_tax_label}{l s='Total (tax excl.)'}{else}{l s='Total'}{/if}</td>
						<td colspan="2" class="price no-border-top" id="total_price_without_tax">{displayPrice price=$total_price_without_tax}</td>
					</tr>-->
					{/if}
					{*
					<tr class="cart_total_tax">
						<td colspan="3" class="tfoot-label no-border">{l s='Tax'}</td>
                ...
                <td  colspan="2" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									<!-- <form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											<span class="label">{l s='APPLY A COUPON CODE'}</span>
											<input type="text" class="discount_name form-control" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
											<input type="hidden" name="submitDiscount" />
											<button type="submit" name="submitAddDiscount" class="button-red button-small"><span>{l s='APPLY'}</span></button>
										</fieldset>
									</form> -->
...
<script type="text/javascript">
	$('document').ready(function(){
		var hasProof = '{$hasProof}';
		var voucher_num = '{$voucher_num}';
		var isPerks = '{$isPerks}';
console.log(isPerks);
