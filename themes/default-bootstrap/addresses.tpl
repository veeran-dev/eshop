	...
	{foreach from=$multipleAddresses item=address name=myLoop}
    	<div class="col-xs-12 col-sm-6 address">
			<ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if}{if $smarty.foreach.myLoop.index % 2} alternate_item{else} item{/if} box">
                ...
                <a class="btn btn-default button button-small" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" data-id="addresses_confirm" data-toggle="modal" data-target="#deleteConfirm" title="{l s='Delete'}"><span>{l s='Delete'}</span></a></li>
            </ul>
        </div>
...
<div class="clearfix main-page-indent">
	<a href="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" title="{l s='Add an address'}" class="button-red"><span>{l s='Add a new address'}</span></a>
</div>
<ul class="footer_links clearfix">
	<li><a class="button button-small" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}"><span> {l s='Back to your account'}</span></a></li>
	<li>
    <a class="button button-small" 
        {if $isPerks == 1} 
            href="{$base_dir}perksDeals" 
        {else} 
