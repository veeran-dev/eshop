...
<div class="box box-small clearfix">
	...
	{/if}
	<p class="dark">
		<strong>{l s='Order Reference %s - placed on' sprintf=$order->id} {dateFormat date=$order->date_add full=0}</strong>
	</p>
</div>
...
<h1 class="page-heading">{l s='Order Tracking'}</h1>
<div class="table_block">
	<table class="detail_step_by_step table table-bordered">
		<thead>
			<tr>
				<th class="first_item">{l s='Time'}</th>
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6">
			<ul class="address item {if $order->isVirtual()}full_width{/if} box">
				<li><h3 class="page-subheading">{l s='Invoice address'}</h3></li>
...
{if $return_allowed}
	<div id="returnOrderMessage">
		...
		<p class="form-group">
			<textarea class="form-control" cols="67" rows="4" name="returnText" placeholder="Type your message here"></textarea>
		</p>
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-default button button-small"><span>{l s='Make an RMA slip'}</span></button>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<p class="form-group">
			<textarea class="form-control" cols="67" rows="4" name="msgText" placeholder="Type your message here"></textarea>
		</p>
		<div class="submit">
