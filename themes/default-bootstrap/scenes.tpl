...
<div id="scenes">
	...
	<div class="thumbs_banner" style="height:{$thumbSceneImageType.height}px;">
		...
		<div id="scenes_list">
			...
			{foreach $scenes as $scene}
				<li id="scene_thumb_{$scene->id}" style="{if !$scene@last} padding-right:10px;{/if}">
					<a style="width:{$thumbSceneImageType.width}px; height:{$thumbSceneImageType.height}px" title="{$scene->name|escape:'html':'UTF-8'}" href="{$link->getPageLink('shop', true)|escape:'html':'UTF-8'}" data-id_scene="{$scene->id|intval}" class="scene_thumb">
		...
		<span class="space-keeper">
			<a class="next" href="{$link->getPageLink('shop', true)|escape:'html':'UTF-8'}" style="height:{math equation='a+2' a=$thumbSceneImageType.height}px;">&nbsp;</a>
		</span>
	</div>
	{/if}
</div>
