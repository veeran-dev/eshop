...
{if isset($p) AND $p}
	...
	<div id="pagination{if isset($paginationId)}_{$paginationId}{/if}" class="pagination clearfix">
	    {if $nb_products > $products_per_page  && $start!=$stop }
			<form class="showall {if $nb_products > 200}hidden{else}{/if}" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get">
				<div>
			...
			<ul class="pagination">
				{if $p != 1}
					...
					<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous">
						<a{$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}" rel="prev">{l s='Previous'}
						</a>
					</li>
				{else}
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next">
						<a{$no_follow_text} href="{$link->goPage($requestPage, $p_next)}" rel="next">{l s='Next'}</i>
						</a>
					</li>
				{else}
