...
<div id="container-customer">
	<div class="row">
					...
					{if $count_ko}
						<table class="table">
							...
							<tbody>
								...
								<tr onclick="document.location = '?tab=AdminOrders&amp;id_order={$order['id_order']|intval}&amp;vieworder&amp;token={getAdminToken tab='AdminOrders'}'">
									<td>{$order['id_order']}</td>
									<td><a href="?tab=AdminOrders&amp;id_order={$order['id_order']}&amp;vieworder&amp;token={getAdminToken tab='AdminOrders'}">{$order['date_add']}</a></td>
									<td>{$order['payment']}</td>
									<td>{$order['order_state']}</td>
									<td>{$order['nb_products']}</td>
									<td>{$order['total_paid_real']}</td>
		...
		<div class="col-lg-6">
			<div class="panel">
				<div class="panel-heading">
					<i class="icon-user"></i> Relationship Manager
				</div>
				<form method="post" id="submitRelationshipManger">
					<div class="form-group">					
						<select id="selectRelationshipManager">
							{if !isset($customer->id_relationship_manager)}
								<option>Select Relationship Manager</option>
							{/if}
							{foreach $relationshipManager as $rm}
								<option value="{$rm.id_employee}" {if $rm.id_employee==$customer->id_relationship_manager}selected="selected"{/if} >{$rm.firstname}</option>
							{/foreach}
						</select>
					</div>
					<div class="row">
						<button type="submit" id="submit_rm" class="btn btn-default pull-right">
							<i class="icon-save"></i>
							Save
						</button>
					</div>
...
$(document).ready(function(){
	...
	$("#submit_rm").click(function(event){
		event.preventDefault();
		submitRM();
	});
	function submitRM()
	{
		var relationshipManger = $("#selectRelationshipManager option:selected").val();
		$.ajax({
				url:'ajax.php',
				type: 'POST',
				async: false,
				dataType: 'html',
				data: {
					submitRelationshipManger : relationshipManger,
				...
				success: function(result)
				{
					if (result == "ok")
					{
						$("#ajax_confirmation").html("Customer Relationship manager updated successfuly");
