...
{block name="override_tpl"}
	...
	<div class="row">
		<div class="col-lg-9">
			<div class="panel">
				...
				<div class="well hidden-print">
					...
					&nbsp;
					{if $order->po_number}
					<a class="btn btn-default" href="{$file_po}">
						<i class="icon-file"></i>
						{$order->po_number}
					</a>
					&nbsp;
					...
					{if Configuration::get('PS_INVOICE') && $consolidated_invoice!='' && $order->invoice_number}
						<a data-selenium-id="view_invoice" class="btn btn-default _blank" href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generateConsolidatedInvoicePdf{$consolidated_invoice}">
							<i class="icon-file"></i>
							{l s='View invoice'}
						</a>
		...
		<div class="col-lg-3">
			...
			<div class="panel-heading">
				...
				{if $delivery_slip}
				{foreach $delivery_slip as $dr}
					<form enctype="multipart/form-data" action="{$currentIndex}&view{$table}&token={$smarty.get.token|escape:'html':'UTF-8'}" method="post" style="margin-top:10px;">
						<a href="#" onclick="$('#pdf-dr-label-{$dr['id_delivery']}').slideToggle();">
							<i class="icon-truck"></i>
						{$dr['delivery_number']}</a>						
						
						<div id="pdf-dr-label-{$dr['id_delivery']}" style="display:none;padding:5px;border:1px solid #DFD5C3;margin-top:5px;">
							<div class="upload-scan" style="padding: 1px 4px 10px;border-bottom:1px solid #dfd5c3;">
								<input type="file" name="file" id="file">
								<input type="hidden" name="id_order" value="{$order->id}" />
								<input type="hidden" name="id_delivery" value="{$dr['id_delivery']}" />
								<input type="hidden" name="dr_number" value="{$dr['delivery_number']}" />
								<input type="submit" name="uploadDRScan" value="Upload" class="button">
							</div>
							<div style="padding: 10px;">
								<a style="padding: 4px;" href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generatePartialDeliverySlipPDF&amp;id_order={$order->id|intval}&amp;id_delivery={$dr['id_delivery']}">
									<i class="icon-download-alt icon-1"></i> DR
								</a>&nbsp;&nbsp;&nbsp;
								<a style="padding: 4px;" href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generateDeliveryLabel&amp;id_delivery_label={$dr['id_delivery']}">
								<i class="icon-download-alt icon-1"></i> Label
								</a>

								{if isset($dr['dr_file_name']) && $dr['dr_file_name']!=''}
									<a href="../scanedDRs/{$dr['dr_file_name']}" style="padding: 4px;" href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generateDeliveryLabel&amp;id_delivery_label={$dr['id_delivery']}">
							...
							<div style="padding:10px;border-top: 1px solid #dfd5c3;margin-top: 10px;">
								...
								<div class="margin-form">
									<input type="text" class="kob_box" name="kob_box" value={$dr['kob_boxes']}>
								</div>
								<label for="other_box" class="clear">Other Labels: </label>
								<div class="margin-form">
									<input type="text" class="other_box" name="other_box" value={$dr['other_boxes']}>
								</div>
								<div class="margin-form">
									<input type="hidden" value={$dr['id_delivery']} name="id_delivery">
				...
				<div>
					<form action="{$currentIndex}&view{$table}&token={$smarty.get.token|escape:'html':'UTF-8'}" method="post" style="margin-top:10px;">
		...
		<div class="col-lg-3">
			<div class="panel">
			<form action="{$currentIndex}&view{$table}&amp;id_order={$order->id}&token={$smarty.get.token|escape:'html':'UTF-8'}" method="post">
				<div class="panel-heading">{l s='Fulfilment Center'}</div>
				<select name="fcentre">
				<option>Select Center</option>
				{foreach $fcOrders as $fc}
					<option value="{$fc['id_fulfillment_centre']}" {if $fc['id_fulfillment_centre']==$order->id_fc}selected="selected"{/if} >{$fc['city_name']}</option>
				{/foreach}
					...
					<div class="panel-heading">
						...
						<span class="badge">
							{if ($customer->verification_status == 0)}
								<span style='color:#e74740;'>{l s='Not Verified'}</span>
							{elseif ($customer->verification_status == 1)}
								<span style='color:#5483f1;'>{l s='Verification Pending'}</span>
							{else}
								<span style='color:#31b442;'>{l s='Verified Customer'}</span>
							{/if}
						</span>
					</div>
					<div class="row">
	...
	<div class="row" id="start_products">
		<div class="col-lg-12">
			<form id="productListOrder" class="container-command-top-spacing" action="{$current_index}&amp;vieworder&amp;token={$smarty.get.token|escape:'html':'UTF-8'}&amp;id_order={$order->id|intval}" method="post" onsubmit="return orderDeleteProduct('{l s='This product cannot be returned.'}', '{l s='Quantity to cancel is greater than quantity available.'}');">
				<input type="hidden" name="id_order" value="{$order->id}" />
						...
						<table class="table" id="orderProducts">
							<thead>
								<tr>
									...
									{/if}
									<th class="text-center"><span class="title_box ">{l s='Delivered quantity'}</span></th>
									<th>
										<span class="title_box ">{l s='Total'}{$order->hasBeenShipped()}{$order->hasBeenDelivered()}</span>
										<small class="text-muted">{$smarty.capture.TaxMethod}</small>
									</th>
									{if !$order->hasBeenDelivered() &&($order->hasBeenPartiallyShipped() || $order->hasBeenShipped() || $order->packedandreadytobeshipped())}
									<th>
