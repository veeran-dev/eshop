...
<div id="product-informations" class="panel product-tab">
	...
	<div class="form-group">
		<label class="control-label col-lg-3 required" for="hsnCode">
			<span class="label-tooltip" data-toggle="tooltip"
				title="{l s='The unique identification code to find type of product. This is for GST purpose.'}">
				{$bullet_common_field} {l s='HSN Code'}
			</span>
		</label>
		<div class="col-lg-3">
			<input maxlength="8" minlength="4" type="text" id="hsnCode" name="hsn_code" value="{$product->hsn_code|htmlentitiesUTF8}" />
		</div>
	</div>
...
<div id="product_options" class="form-group">
	...
	<div class="form-group">
		...
		<div class="col-lg-9">
			<div class="checkbox">
				<label for="zone">
					<input type="checkbox" class="zone" name="zone[]" id="zone[]" value="0" {if $available_across_india == 1}checked="checked"{/if} >PAN India</label>
			</div>
			{foreach $zones as $zone}
				<div class="checkbox">
					<label for="zone">
						<input type="checkbox" class="zone" name="zone[]" id="zone[]" value="{$zone['id_fulfillment_centre']}" {if $zone['available']}checked="checked"{/if} >
						{$zone['city_name']}</label>
				</div>
			{/foreach}
		</div>
