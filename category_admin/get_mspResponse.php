...
$query 					= "SELECT `list_most_searched` FROM `"._DB_PREFIX_."category_listing` WHERE `category_id` = ".$id_category;
$category_details	 	= Db::getInstance()->ExecuteS($query);
$list_category 			= unserialize($category_details[0]['list_most_searched']);

if (isset($_POST["action"]) && $_POST["action"] == 'add' ){
	
	$list_category[] = $_POST["list_most_searched"];
...
if (isset($_POST["action"])){
	...
	$update_query 	= "UPDATE `"._DB_PREFIX_."category_listing` SET list_most_searched = '".$updateData."' WHERE `category_id` = ".$id_category;
...
<ol>
<?php
$msp_list = implode(',',$list_category);
