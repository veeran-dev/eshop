...
$query 					= "SELECT `list_kobster_recommented` FROM `"._DB_PREFIX_."category_listing` WHERE `category_id` = ".$id_category;
$category_details	 	= Db::getInstance()->ExecuteS($query);
$list_category 			= unserialize($category_details[0]['list_kobster_recommented']);

if (isset($_POST["action"]) && $_POST["action"] == 'add' ){
	
	$list_category[] = $_POST["list_kobster_recommented"];
...
if (isset($_POST["action"])){
	...
	$update_query 	= "UPDATE `"._DB_PREFIX_."category_listing` SET list_kobster_recommented = '".$updateData."' WHERE `category_id` = ".$id_category;
...
?>
    ...
    <li><?php echo $products['product_name']; ?> <span class="slider_remove" onClick="kobster_recommented(<?php echo $products['id_product']; ?>, 'delete');"><i class="fa fa-trash-o"></i></span> </li>
