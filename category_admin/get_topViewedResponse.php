...
$query 					= "SELECT `list_top_viewed` FROM `"._DB_PREFIX_."category_listing` WHERE `category_id` = ".$id_category;
$category_details	 	= Db::getInstance()->ExecuteS($query);
$list_category 			= unserialize($category_details[0]['list_top_viewed']);

if (isset($_POST["action"]) && $_POST["action"] == 'add' ){
	
	$list_category[] = $_POST["list_top_viewed"];
...
if (isset($_POST["action"])){
	...
	$update_query 	= "UPDATE `"._DB_PREFIX_."category_listing` SET list_top_viewed = '".$updateData."' WHERE `category_id` = ".$id_category;
...
?>
    ...
    <li><?php echo $products['product_name']; ?> <span class="slider_remove" onClick="top_viewed(<?php echo $products['id_product']; ?>, 'delete');"><i class="fa fa-trash-o"></i></span> </li>
