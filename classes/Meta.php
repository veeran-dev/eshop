...
class MetaCore extends ObjectModel
{
    ...
    public static function getPages($exclude_filled = false, $add_page = false)
    {
        ...
        if (!$override_supplier_files = Tools::scandir(_PS_CORE_DIR_.DIRECTORY_SEPARATOR . 'override' . DIRECTORY_SEPARATOR . 'controllers' . DIRECTORY_SEPARATOR . 'supplier' . DIRECTORY_SEPARATOR, 'php', '', true)) {
            die(Tools::displayError('Cannot scan "override" directory'));
        }
        
        $files = array_values(array_unique(array_merge($files, $override_files, $override_supplier_files)));
