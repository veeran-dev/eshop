...
class EmployeeCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'rm_head' =>                    array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'sales_head' =>                    array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
	...
	const FLAG_FINANCE_TEAM	= 11;
	const FLAG_RELATIONSHIP_MANAGER	= 5;
	const FLAG_SUPPLY_CHAIN	= 6;
    ...
    public function smartCheckEmail($email){
        $result = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'employee` WHERE `email` = \''.pSQL($email).'\'');
        if (!$result) {
            return false;
        }
        $this->id = $result['id_employee'];
    ...
    public static function hasCode($pin){
        return Db::getInstance()->getRow('SELECT id_employee from `'._DB_PREFIX_.'employee_pin` WHERE pin='.$pin);
    }

    public static function generatePin($id_employee){
        $digits = 4;
        
        $digit = rand(pow(10, 4-1), pow(10, 4)-1);
        while (Employee::hasCode($digit)) {
            $digit = rand(pow(10, 4-1), pow(10, 4)-1);
        }
        if(Employee::addPin($id_employee, $digit)){
            return $digit;
        }
    }

    public static function addPin($id_employee, $digit){
        $check = Db::getInstance()->getRow('SELECT * from `'._DB_PREFIX_.'employee_pin` WHERE id_employee='.$id_employee);
        if(!$check){
            $sql ='INSERT INTO `kob_employee_pin` (`id_employee`, `pin`) VALUES ('.$id_employee.', '.$digit.')';
        }
        else{
            $sql ='UPDATE `kob_employee_pin` set pin='.$digit.' WHERE id_employee='.$id_employee.'';
        }
        
        return Db::getInstance()->execute($sql);
    }

    public function smartLogin($pin)
    {

        $result = Db::getInstance()->getRow('SELECT * FROM `'._DB_PREFIX_.'employee` WHERE id_employee IN(SELECT id_employee from `'._DB_PREFIX_.'employee_pin` WHERE pin='.$pin.')');
        if (!$result) {
            return false;
        }
        $this->id = $result['id_employee'];
