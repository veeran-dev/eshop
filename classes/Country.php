...
class CountryCore extends ObjectModel
{
    ...
    public static function getCountries($id_lang, $active = false, $contain_states = false, $list_states = true)
    {
        ...
        if ($list_states) {
            ...
            if($context->cookie->is_perks)
            {
                $sub_query = 'SELECT s.* FROM `'._DB_PREFIX_.'fulfillment_centre` fc LEFT JOIN `'._DB_PREFIX_.'state` s ON fc.id_state = s.id_state ORDER BY `name` ASC';
                $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS($sub_query);
            }
            else
            {
                $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'state` s '.$sub_query.' ORDER BY `name` ASC');
            }
    ...
    {
        $state_list= Db::getInstance()->ExecuteS('
