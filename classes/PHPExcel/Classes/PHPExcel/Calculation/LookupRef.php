	...
	public static function COLUMN($cellAddress=Null) {
		...
		if (is_array($cellAddress)) {
			foreach($cellAddress as $columnKey => $value) {
				$columnKey = preg_replace_callback('/[^a-z]/i','',$columnKey);
			...
			if (strpos($cellAddress,':') !== false) {
				...
				$startAddress = preg_replace_callback('/[^a-z]/i','',$startAddress);
				$endAddress = preg_replace_callback('/[^a-z]/i','',$endAddress);
				$returnValue = array();
				do {
				...
				$cellAddress = preg_replace_callback('/[^a-z]/i','',$cellAddress);
		...
		if (is_array($cellAddress)) {
			foreach($cellAddress as $columnKey => $rowValue) {
				foreach($rowValue as $rowKey => $cellValue) {
					return (integer) preg_replace_callback('/[^0-9]/i','',$rowKey);
				}
			}
		} else {
			if (strpos($cellAddress,'!') !== false) {
			...
			if (strpos($cellAddress,':') !== false) {
				list($startAddress,$endAddress) = explode(':',$cellAddress);
				$startAddress = preg_replace_callback('/[^0-9]/','',$startAddress);
				$endAddress = preg_replace_callback('/[^0-9]/','',$endAddress);
				$returnValue = array();
				do {
				...
				return (integer) preg_replace_callback('/[^0-9]/','',$cellAddress);
			}
		}
	}	//	function ROW()
