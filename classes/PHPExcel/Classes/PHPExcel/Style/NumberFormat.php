...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		$format = preg_replace_callback('/^(\[\$[A-Z]*-[0-9A-F]*\])/i', '', $format);
	...
	public static function toFormattedString($value = '0', $format = PHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		$color_regex = '/^\\[[a-zA-Z]+\\]/';
		$format = preg_replace_callback($color_regex, '', $format);
		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime format
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				$format = preg_replace_callback('/_./', '', $format);

				// Some non-number characters are escaped with \, which we don't need
				$format = preg_replace_callback("/\\\\/", '', $format);
...
//				$format = str_replace(array('\\"', '*'), array('"', ''), $format);
				...
				if ($useThousands) {
					$format = preg_replace_callback('/0,0/', '00', $format);
					$format = preg_replace_callback('/#,#/', '##', $format);
				}
				...
				if (preg_match('/(#|0)(,+)/', $format, $matches)) {
					...
					// strip the commas
					$format = preg_replace_callback('/0,+/', '0', $format);
					$format = preg_replace_callback('/#,+/', '#', $format);
				}
				...
				if (preg_match('/#?.*\?\/\?/', $format, $m)) {
					...
					// Strip #
					$format = preg_replace_callback('/\\#/', '0', $format);

					$n = "/\[[^\]]+\]/";
					$m = preg_replace_callback($n, '', $format);
					$number_regex = "/(0+)(\.?)(0*)/";
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							...
							$value = preg_replace_callback($number_regex, $value, $format);
						} else {
				...
				if (preg_match('/\[\$(.*)\]/u', $format, $m)) {
					...
					$value = preg_replace_callback('/\[\$([^\]]*)\]/u',$currencyCode,$value);
				}
			}
		}
