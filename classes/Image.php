...
class ImageCore extends ObjectModel
{
    ...
    public function getPathForCreation()
    {
        ...
        $logger->logError('getPathForCreation');
        if (!$this->id) {
            $logger->logError('return false');
            return false;
        }
        if (Configuration::get('PS_LEGACY_IMAGES')) {
            $logger->logError('return false 810');
            if (!$this->id_product) {
                $logger->logError('return false 812');
                return false;
            }
            $path = $this->id_product.'-'.$this->id;
        } else {
            $logger->logError('return false 817');
