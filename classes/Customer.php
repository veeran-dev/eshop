...
class CustomerCore extends ObjectModel
{
	...
	/******************************Created By Sreeni added the customize variables for 1.6.1.3*********************/
	 
	public     $mobile;
	/** @var integer buyer ID */
	public		$id_buyer = 9;
	
	/** @var varchar buyer ID */
	public 		$verification_document;
	
	/** @var integer verification_status */
	public      $verification_status;
	
	/** @var integer verified_by */
	public      $verified_by;
	
	/** @var datetime verification_alert_sent */
	public      $verification_alert_sent;
	
	/** @var integer id_relationship_manager */
	public 		$id_relationship_manager;

    /** @var boolean is_logged customer **/
    public $is_logged;
    ...
    public static $definition = array(
        ...
        'fields' => array(
			...
			/******************************Created By Sreeni added the customize variables for 1.6.1.3*********************/
			'id_buyer' =>    				 array('type' => self::TYPE_INT),
            'mobile' =>                    array('type' => self::TYPE_INT),
            'id_relationship_manager' =>     array('type' => self::TYPE_INT),
            'verification_document' =>       array('type' => self::TYPE_STRING),
            'verification_status' =>         array('type' => self::TYPE_INT),
            'verified_by' =>                 array('type' => self::TYPE_INT),
            'verification_alert_sent' =>     array('type' => self::TYPE_DATE),
            'is_logged' =>     array('type' => self::TYPE_BOOL),
            'epaylater' =>                 array('type' => self::TYPE_INT),
            'id_proof' =>                    array('type' => self::TYPE_STRING, 'size' => 12),
			/******************************End of customize variables for 1.6.1.3*********************/
        ),
    );
    ...
    public function getAddresses($id_lang)
    {
		...
		$ids=$this->getChildCustomers();
        
        if( (int)$this->getCustomerRole() != 2 && (int)$this->getCustomerRole() != 3 ){
            if($this->id_buyer==3){
                $query = 'b.`id_customer` = '.(int)($this->id).'';
            }
            else{
                $query = 'a.`id_customer` = '.(int)($this->id).'';
            }
        }
        else{

            if($this->id_buyer==3){
               $query = 'b.`id_customer` IN ('.$ids.')';
            }
            elseif($this->id_buyer == 10)
            {
                $query = 'a.`id_customer` = '.(int)($this->id).'';
            }
            else{
               $query = 'a.`id_customer` IN ('.$ids.')'; 
            }
        }
                $join="";
        if($this->id_buyer==3){
            $join="LEFT JOIN `"._DB_PREFIX_."customer_address` b on (a.id_address = b.id_address)";
        }
        ...
        if (!Cache::isStored($cache_id)) {
            $sql = 'SELECT DISTINCT a.*, cl.`name` AS country, s.`name` AS state, 
                    s.`iso_code` AS state_iso, sfm.`id_fulfillment_centre` AS delivery_region
					FROM `'._DB_PREFIX_.'address` a
					'.$join.'
					LEFT JOIN `'._DB_PREFIX_.'country` c ON (a.`id_country` = c.`id_country`)
					LEFT JOIN `'._DB_PREFIX_.'country_lang` cl ON (c.`id_country` = cl.`id_country`)
					LEFT JOIN `'._DB_PREFIX_.'state` s ON (s.`id_state` = a.`id_state`)
                    LEFT JOIN `'._DB_PREFIX_.'state_fc_mapping` sfm ON (a.`id_state` = sfm.`id_state`)
					'.($share_order ? '' : Shop::addSqlAssociation('country', 'c')).'
					WHERE `id_lang` = '.(int)$id_lang.' AND '.$query.' AND a.`deleted` = 0 group by a.id_address';
