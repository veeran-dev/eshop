...
class OrderCore extends ObjectModel
{
   ...
   */
	...
	*/
	...
	{		$child= new Customer($id_customer);		$child_id = $child->getChildCustomers();		return (int)Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT `id_cart`			FROM `'._DB_PREFIX_.'orders`			WHERE `id_order` = '.$id_order.' and `id_customer` in ('.$child_id.')');
	...
	*/
	...
	public static function getHistoryReport($customerid, $duration, $addressId)
	{
		...
		}				else if($duration == 6)// Get Current Month Report
		{
			$due_query = 'AND MONTH(a.date_add) ='.$month.'';
		}
		else 
		{
			$due_query ='';
