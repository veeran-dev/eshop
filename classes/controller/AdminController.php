...
class AdminControllerCore extends Controller
{
    ...
    public function processStatus()
    {
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            if ($object->toggleStatus()) {
                if($object->active == false && Tools::getValue('id_product')){
                    $content['0'] = array(
                        'from' => 'enable',
                        'to' => 'disable',
                        );
                    $catalogHistory = new CatalogHistory();
                    $catalogHistory->addHistory("status", Tools::getValue('id_product'), $content);
                }
                $matches = array();
    ...
    public function initContent()
    {
        ...
        $cookie = Context::getContext()->cookie;
        if($cookie->profile == 5){
            Tools::redirect('index.php?controller=rm-login?bypass=1&SubmitRMLogin=1&id_employee='.$cookie->id_employee.'');
        }
        if($cookie->profile == 10){
            Tools::redirect('index.php?controller=scn-login?bypass=1&SubmitScnLogin=1&id_employee='.$cookie->id_employee.'');
        }
        $this->getLanguages();
        $this->initToolbar();
    ...
    public function init()
    {
        ...
        if (isset(Context::getContext()->cookie->last_activity) && !$this->logged_internal) {
            if ($this->context->cookie->last_activity + 1200 < time()) { // 20 seconds inactivity -> logout
                $this->context->employee->logout();
            } else {
    ...
    public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false)
    {
        ...
        do {
			...
			'.(isset($this->_select) ? ', '.rtrim($this->_select, ', ') : '').$select_shop;
 			if($this->_filter)
			{
 				$this->_filter =str_ireplace("`customer`","CONCAT(c.`firstname`)",$this->_filter);
			}
            
            if(preg_match('/AND c.`id_buyer` = 0/',$this->_filter ))
            {
                $this->_filter =str_ireplace("AND c.`id_buyer` = 0","AND c.`id_buyer` NOT IN(3) ",$this->_filter);
            }
