...
class FrontControllerCore extends Controller
{
    ...
    public function __construct()
    {
        ...
        if(isset($this->context->cookie->id_customer)) {
            ...
            $customerObj->setLastConnectionDate();
            if(!(int)$customerObj->is_logged && $customerObj->is_logged != "") {
                if (isset(Context::getContext()->cookie)) {
                    // update log value to customer table logoff a session
                    Db::getInstance()->autoExecute(_DB_PREFIX_.'customer', 
                        array('is_logged' => 1), 
                        'UPDATE', 
    ...
    public function init()
    {
        ...
        $protocol_content = Tools::getShopProtocol();
        ...
        if ($id_cart = (int)$this->recoverCart()) {
          ...
          if ($this->auth && !$this->context->customer->isLogged($this->guestAllowed)) {
            ...
            else if($this->page_name == 'perksDeals')
            {
                Tools::redirect('index.php?controller=perks');   
            }
			else
        ...
        $cookie->c_char = $currency->sign;
        $cookie->c_format = $currency->format;
        $cookie->c_decimals = (int)$currency->decimals * _PS_PRICE_DISPLAY_PRECISION_;
        $cookie->c_blank = $currency->blank;
        ...
        if (isset($_GET['logout']) || ($this->context->customer->logged && Customer::isBanned($this->context->customer->id))) {
            ...
            if($this->context->cookie->is_perks == 1)
            {
            ...
            {
                $this->context->customer->mylogout();
                Tools::redirect($protocol_content.Tools::getHttpHost().__PS_BASE_URI__."dash-login.php");
            }
        }
        elseif (isset($_GET['logoutView'])) {
            $this->context->customer->mylogout();
            Tools::redirect('PinCap/');
        }
        ...
        if ((int)$this->context->cookie->id_cart) {
            ...
            /* If cart products already exists and not having delivery region  - set flag*/
            if(isset($cart) && count($cart->getProducts()) && !$cart->OrderExists() && $this->context->customer->id_buyer!= 10) {
                if($this->context->cookie->id_state != "") {
                    $cart->region = intval($this->context->cookie->id_state);
                    $cart->update();  
                }
                
                if(!$cart->region && count($states) > 1) {
                    $cart_region_not_exists = true;
                }
                else {
                    $state = new State(intval($cart->region));
                    $products = $cart->getProducts();
                    $fc = new FulfillmentCentre(intval($state->getFcIdByState()));
                    $is_rm_request = (int)Tools::getValue('rm_request_no_delete');
                    if(!empty($products)) {
                        foreach ($products as $key => $product) {
                            $productObj = new Product(intval($product['id_product']));
                            if(isset($fc->id) && !$fc->getProductAvailable($productObj->id)) {
                                if(!$is_rm_request) {
                                    $cart->deleteProduct($productObj->id, $product['id_product_attribute'], $product['id_customization'], $product['id_address_delivery']);
                                }
                            }
                        }
                    }
                    $this->context->cookie->delivery_region = $state->getFcIdByState();
                    $this->context->cookie->delivery_region_name = $state->name;
                    $this->context->cookie->id_state = (int)$state->id;
                }
            }
        }
        ...
        $isPerks = 0;
        if($this->context->cookie->id_buyer == 10)
        {
            $isPerks = 1;
            $this->context->cookie->is_perks = 1;
        }
        
        $noSales = $this->getShopSaleStatus();
        ...
        $this->context->smarty->assign(array(
            ...
            'isPerks'             => $isPerks,
            'noSales'             => $noSales,
            'hasProof'            => $this->context->customer->id_proof != '' ? true:false,
            'mode'                => (string)Configuration::get('MODE'),
            'cart_region_not_exists' => (int)$cart_region_not_exists
        ));
    ...
    public function isTokenValid()
    {
        ...
        return (strcasecmp(Tools::getToken(false), Tools::getToken(false)) == 0);
    }
