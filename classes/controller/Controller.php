...
abstract class ControllerCore
{
    ...
    public function __construct()
    {
        ...
        $logger->logError("controller construct");
        $logger->logError($this->php_self);
        $logger->logError($this->context->customer->id);
    ...
    public function run()
    {
        $logger = new FileLogger();
        $logger->SetFilename("test.txt");
        $logger->logError("controller run");
        $this->init();
        if ($this->checkAccess()) {
            // setMedia MUST be called before postProcess
            if (!$this->content_only && ($this->display_header || (isset($this->className) && $this->className))) {
        $logger->logError("controller 183");        $this->setMedia();
            }
            ...
            if (!empty($this->redirect_after)) {
                $logger->logError("controller 190"); $this->redirect();
            }

            if (!$this->content_only && ($this->display_header || (isset($this->className) && $this->className))) {
                $logger->logError("controller 194");$this->initHeader();
            }

            if ($this->viewAccess()) {
                $logger->logError("controller 198");$this->initContent();
            } else {
                $this->errors[] = Tools::displayError('Access denied.');
            }

            if (!$this->content_only && ($this->display_footer || (isset($this->className) && $this->className))) {
                $logger->logError("controller 204");$this->initFooter();
            }

            // Default behavior for ajax process is to use $_POST[action] or $_GET[action]
            // then using displayAjax[action]
            if ($this->ajax) {$logger->logError("controller 209");
            ...
            } else {$logger->logError("controller 217");
                $this->display();
            }
        } else {$logger->logError("controller 220");
            $this->initCursedPage();
