...
abstract class ObjectModelCore implements Core_Foundation_Database_EntityInterface
{
     ...
     * @throws PrestaShopException
     */
    public function validateFields($die = true, $error_return = false, $dash = NULL)
    {
        foreach ($this->def['fields'] as $field => $data) {
            if (!empty($data['lang'])) {
                continue;
        ...
        if (!in_array('validate', $skip) && !empty($data['validate'])) {
            ...
            if (!empty($value)) {
                ...
                if (Tools::strtolower($data['validate']) == 'iscleanhtml') {
                    ...
                    //$logger->logError($data['validate']);
                    //$logger->logError($value);
                    //$logger->logError(call_user_func(array('Validate', $data['validate']), $value));
