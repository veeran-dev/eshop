...
class CartCore extends ObjectModel
{
    ...
    /** @var int last added delivery region */
    public $region;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'region' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
     ...
     */
    public function getProducts($refresh = false, $id_product = false, $id_country = null, $id_supplier = null)
    {
        if (!$this->id) {
            return array();
        }
        ...
        if ($this->_products !== null && !$refresh) {
            ...
            $logger->logError("retrn tProducts");
            
            $logger->logError($this->_products);
        ...
        // Build SELECT
        $sql->select('cp.`id_product_attribute`,cp.`id_supplier`,kes.name AS supplier_name, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, pl.`name`, p.`is_virtual`,
                        pl.`description_short`, pl.`available_now`, pl.`available_later`, product_shop.`id_category_default`,kess.out_of_stock,
        ...
        $sql->leftJoin('elite_supplier', 'kes', 'kes.id_supplier=cp.id_supplier');
        $sql->leftJoin('elite_supplier_stock', 'kess', 'kess.id_supplier=cp.id_supplier AND kess.`id_product` = cp.`id_product`');
        ...
        $sql->where('kess.`id_stock` IN(select MAX(id_stock) from kob_elite_supplier_stock where id_product=cp.id_product and id_supplier=cp.id_supplier)');
        if($id_supplier) {
            $sql->where('cp.`id_supplier` = '.(int)$id_supplier);
        }

        // Build ORDER BY
        $sql->orderBy('cp.`date_add`, cp.`id_product`, cp.`id_product_attribute`,kes.id_supplier ASC');
...
$logger->logError($row);
    ...
    public function containsProduct($id_supplier, $id_product, $id_product_attribute = 0, $id_customization = 0, $id_address_delivery = 0)
    {
        ...
        $sql .= '
            ...
            AND cp.`id_supplier`='.(int)$id_supplier.' 
     ...
     */
    public function updateQty($quantity, $id_supplier, $id_product, $id_product_attribute = null, $id_customization = false,
        $operator = 'up', $id_address_delivery = 0, Shop $shop = null, $auto_add_cart_rule = true)
    {
        ...
        if ((int)$quantity <= 0) {
            return $this->deleteProduct($id_supplier, $id_product, $id_product_attribute, (int)$id_customization);
        } elseif ((Configuration::get('PS_CATALOG_MODE') && !defined('_PS_ADMIN_DIR_'))) {
            return false;
        } else {
            /* Check if the product is already in the cart */
            $result = $this->containsProduct($id_supplier, $id_product, $id_product_attribute, (int)$id_customization, (int)$id_address_delivery);
            ...
            if ($result) {
                if ($quantity < $minimal_quantity) {
                    ...
                    Db::getInstance()->execute('
                        ...
                        WHERE `id_supplier`='.$id_supplier.' AND `id_product` = '.(int)$id_product.
    ...
    */
     ...
     * @return bool result
     */
    public function deleteProduct($id_supplier, $id_product, $id_product_attribute = null, $id_customization = null, $id_address_delivery = 0)
    {
        if (isset(self::$_nbProducts[$this->id])) {
            unset(self::$_nbProducts[$this->id]);
        }
        ...
        if ((int)$id_customization) {
            ...
            // refresh cache of self::_products
            $this->_products = $this->getProducts(true);
            return ($customization_quantity == $product_total_quantity && $this->deleteProduct($id_supplier, (int)$id_product, (int)$id_product_attribute, null, (int)$id_address_delivery));
        }
    ...
    */
    public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true)
    {
        ...
        if($buyer_id ==3 AND $type == Cart::ONLY_SHIPPING)
            return 0;
        if ($virtual && $type == Cart::BOTH) {
            $type = Cart::BOTH_WITHOUT_SHIPPING;
        }

        if (($with_shipping || $type == Cart::ONLY_DISCOUNTS) && $buyer_id != 3) {
        ...
        if (!in_array($type, array(Cart::ONLY_SHIPPING, Cart::ONLY_PRODUCTS)) && CartRule::isFeatureActive()) {
            ...
            // Discount calculation should be tax incl always
            $with_taxes_original = $with_taxes;
            $with_taxes = true;
            ...
            $with_taxes = $with_taxes_original;
