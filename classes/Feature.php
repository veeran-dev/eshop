...
class FeatureCore extends ObjectModel
{
    ...
    */
    ...
    {
       //$rq = Db::getInstance()->getRow('SELECT fl.`id_feature` FROM '._DB_PREFIX_.'feature_lang fl left join '._DB_PREFIX_.'feature f on f.id_feature = fl.id_feature`  WHERE fl.`name` = \''.pSQL($name).'\' GROUP BY f.`id_feature`');
		$rq = Db::getInstance()->getRow('SELECT f.`id_feature` FROM '._DB_PREFIX_.'feature f
LEFT JOIN '._DB_PREFIX_.'feature_lang fl ON f.id_feature = fl.id_feature  WHERE fl.`name` = \''.pSQL($name).'\' GROUP BY f.`id_feature`');
        if (empty($rq)) {
