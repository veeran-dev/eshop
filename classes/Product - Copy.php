...
class ProductCore extends ObjectModel
{
	...
	/** @var boolean Product Disable product */
	...
	public function getFields()
	{
		...
		$fields['discontinued'] = (int)($this->discontinued);
	...
	*/
	...
	public static function getProductProperties($id_lang, $row)
	{
		...
		/*$averages = ProductComment::getAveragesByProduct((int)$row['id_product'], (int)$id_lang);
		$averageTotal = 0;
		foreach ($averages AS $average)
			$averageTotal += (float)($average);
		$averageTotal = count($averages) ? ($averageTotal / count($averages)) : 0;
		$row['averageRating'] = $averageTotal;*/
		
		$row['averageRating'] = Product::getRating((int)$row['id_product']);
	...
	{
		return Db::getInstance()->getValue('
		SELECT sum(grade)/count(grade) as rating
	...
	*/
	...
	public function insertUpdateZone($id_product, $id_zone){
		if($id_zone==0)
		{
			Db::getInstance()->Execute('
				DELETE FROM `'._DB_PREFIX_.'product_zone_mapping` WHERE product_id='.$id_product);
		}
		else
		{
			Db::getInstance()->Execute('
				INSERT INTO `'._DB_PREFIX_.'product_zone_mapping` (`product_id`, `zone_id`)
				VALUES ('.$id_product.', '.$id_zone.') 
				ON DUPLICATE KEY UPDATE zone_id='.$id_zone);
		}
	}
	...
	public function getCustomerDefaultAddress($customer)
	{
		 
 		return Db::getInstance()->getValue('SELECT postcode FROM '._DB_PREFIX_.'address WHERE id_customer= '.$customer.'');
 	}
	public function saveOldPrice($id_product,$old_price,$new_price,$id_group,$id_employee)
	{

 		Db::getInstance()->ExecuteS('INSERT INTO `'._DB_PREFIX_.'price_history`(`id_product`, `id_group`, `old_price`, `new_price`, `id_employee`) VALUES ('.$id_product.','.$id_group.','.$old_price.','.$new_price.','.$id_employee.')');
	}
	public function getProductDetail($id_product)
	{
 		return Db::getInstance()->getRow('select p.reference,pl.name as product_name
										from '._DB_PREFIX_.'product as p
										left join '._DB_PREFIX_.'product_lang as pl
										on pl.id_product = p.id_product
										where p.id_product = '.$id_product.' and id_lang=1 ');
 	}
 	public function getProductQuickviewDetails($id_product)
 	{
 		return Db::getInstance()->getRow('SELECT p.*,pl.`description_short`,pl.`name` from '._DB_PREFIX_.'product as p LEFT JOIN '._DB_PREFIX_.'product_lang as pl on p.id_product = pl.id_product where p.id_product = '.$id_product.' and id_lang=1 ');
 	}
}
