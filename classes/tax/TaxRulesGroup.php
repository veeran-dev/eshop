...
class TaxRulesGroupCore extends ObjectModel
{
    ...
    public static function getTaxRulesGroups($only_active = true)
    {
        return Db::getInstance()->executeS('
			SELECT DISTINCT g.id_tax_rules_group, g.name, g.id_tax_rules_group as value, g.name as label, g.active
    ...
    */
    ...
    public static function getTaxesName($id_tax_rules_group)
    {
        return Db::getInstance()->getValue(
            'SELECT `name`
            FROM `'._DB_PREFIX_.'tax_rules_group` rg
            WHERE `id_tax_rules_group` ='.$id_tax_rules_group.''
        );
    }
    ...
    */
    public static function getTaxesRate($id_tax_rules_group, $id_country, $id_state, $zipcode = NULL)
    {
