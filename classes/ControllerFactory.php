...
class ControllerFactoryCore
{
    ...
    public static function includeController($className)
    {
        ...
        if (!class_exists($className, false)) {
			if (file_exists(_PS_CORE_DIR_.'/controllers/'.$className.'.php'))
			{
 				require_once(_PS_CORE_DIR_.'/controllers/'.$className.'.php');
			}
			/*Created By sreeni - hereafter we are going to put all customized controller in the (override/controllers/front) folder. so previously the condition not there. thats y added the addition elseif condition */
            elseif (file_exists(_PS_CORE_DIR_.'/override/controllers/front/'.$className.'.php'))
			{
 				require_once(_PS_CORE_DIR_.'/override/controllers/front/'.$className.'.php');
			}
            /*End of custome elseif*/
            elseif (file_exists(_PS_ROOT_DIR_.'/override/controllers/'.$className.'.php')) {
