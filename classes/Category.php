...
class CategoryCore extends ObjectModel
{
                                                ...
                                                $use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '')
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance()->executeS('
				SELECT c.id_category, cl.name, c.id_category as value, cl.name as label
				FROM `'._DB_PREFIX_.'category` c
        ...
        $use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '')
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance()->executeS('
				SELECT c.*, cl.*, cl.name as label, c.id_category as value,
				FROM `'._DB_PREFIX_.'category` c
				'.($use_shop_restriction ? Shop::addSqlAssociation('category', 'c') : '').'
				LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON c.`id_category` = cl.`id_category`'.Shop::addSqlRestrictionOnLang('cl').'
				'.(isset($groups) && Group::isFeatureActive() ? 'LEFT JOIN `'._DB_PREFIX_.'category_group` cg ON c.`id_category` = cg.`id_category`' : '').'
    ...
    public static function recursive_category_tree($catid=0){
        if(!$catid && $catid != 0){
            return false;
        }
        
        $sql = "SELECT kc.id_category , kcl.name, kc.id_category as value, kcl.name as label, null as children FROM `kob_category` kc
                        LEFT JOIN kob_category_lang kcl ON kcl.id_category=kc.id_category AND kcl.id_lang=1
                        WHERE kc.id_parent=".$catid;
                        
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        // var_dump($sql);
        $recursive_category_tree = array(); 
        
        
        foreach($results as &$result){
            $result['children'] = Category::recursive_category_tree($result['id_category']);
        }
    ...
    public function getProducts($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null)
    {
        ...
        if ($get_total) {
            $sql = 'SELECT COUNT(cp.`id_product`) AS total
					...
					WHERE p.hsn_code !="" AND cp.`id_category` = '.(int)$this->id.
    ...
    public static function getChildren($id_parent, $id_lang, $active = true, $id_shop = false)
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $query = 'SELECT c.`id_category`, cl.`name`, cl.`link_rewrite`, category_shop.`id_shop`, c.`active`
			FROM `'._DB_PREFIX_.'category` c
			LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category`'.Shop::addSqlRestrictionOnLang('cl').')
			'.Shop::addSqlAssociation('category', 'c').'
...
* 	 * @param bool $method_to_create a category
    ...
    */
    ...
    public static function getCategoryInformations($ids_category, $id_lang = null)
    {
		...
		SELECT c.`id_category`, cl.`name`, cl.`link_rewrite`, cl.`id_lang`, c.`active`
		FROM `'._DB_PREFIX_.'category` c
		LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category`'.Shop::addSqlRestrictionOnLang('cl').')
		'.Shop::addSqlAssociation('category', 'c').'
		WHERE cl.`id_lang` = '.(int)$id_lang.'
