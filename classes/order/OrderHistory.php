...
class OrderHistoryCore extends ObjectModel
{
    ...
    public function changeIdOrderState($new_order_state, $id_order, $use_existing_payment = false)
    {
        ...
        // $message = isset($context->employee->firstname) ? $context->employee->firstname.' '.$context->employee->lastname : $cookie->firstname.' '.$cookie->lastname;
        $message = "";
        if($context->cookie->s_id){
            $supplier = new EliteSupplier($context->cookie->s_id);
            $message = $supplier->company;
        }
        else if(isset($context->employee->firstname)){
            $message = $context->employee->firstname.' '.$context->employee->lastname;
        }
        else{
            $message = $cookie->firstname.' '.$cookie->lastname;   
        }
        
        $messageObject = new Message();

        if($old_os->name != "") {
            $message = $message.' changed status from '.$old_os->name.' to '.$new_os->name.'';
            
            $id_employee = isset($context->employee->id) ? (int)($context->employee->id) : (int)($cookie->id_employee);
            
            if($id_employee != "")
                $messageObject->id_employee = isset($context->employee->id) ? (int)($context->employee->id) : (int)($cookie->id_employee);
            else
                $messageObject->id_customer = $order->id_customer;
        }
        else {
            $message = "Initial Status: ".$new_os->name;
            $messageObject->id_customer = $order->id_customer;
        }
        ...
        /**** Change ESTIMATED DELIVERY TIME for an order if placed ****/
        $customer = new Customer($order->id_customer);  
        if($customer->id_buyer == 3 AND $this->id_order_state == 22 AND (int)$customer->getCustomerRole() != 1) {
    ...
    public function addWithemail($autodate = true, $template_vars = false, $internal_mail = false)
    {
        ...
        if (!$this->sendEmail($order, $template_vars, $internal_mail)) {
            return false;
        }
    ...
    public function sendEmail($order, $template_vars = false, $internal_mail = false)
    {
        $result = Db::getInstance()->getRow('
            SELECT osl.`template`, c.`lastname`, c.`firstname`, osl.`name` AS osname, c.`email`, os.`module_name`, os.`id_order_state`, os.`pdf_invoice`, os.`pdf_delivery`, e.`firstname` AS emp_first_name, e.`lastname` AS emp_last_name, e.`email` AS emp_email 
        ...
        $customer = new Customer($order->id_customer);
        if($result['template'] == 'shipped' && $customer->id_buyer == 10)
        {
            $result['template'] = 'perks_shipped';
        }
        if($result['template'] == 'order_canceled' && $customer->id_buyer == 10)
        {
            $result['template'] = 'perks_order_canceled';
        }
        ...
        if (isset($result['template']) && Validate::isEmail($result['email'])) 
        {
            ...
            $data = array(
                            ...
                            '{id_order}'       => (int)$order->getParentOrder(),
                            '{order_name}'     => $order->getUniqReference(),
                            '{employee_name}'  => $result['emp_first_name']." ".$result['emp_last_name'],
                            '{employee_email}' => $result['emp_email'],
                            '{order_status}'   => $result['osname']
                        );
            ...
            if (Validate::isLoadedObject($order)){
                ...
                if(!$internal_mail){
                    ...
                    if (!Mail::Send((int)$order->id_lang, 
                                    ...
                                    $data, 
                                    $template_vars['to_mail'], 
                                    $template_vars['to_name'],
                                    null, 
                                    null, 
                                    $file_attachement, 
                                    null, 
    ...
    public static function checkParentOrderStatus($id_order, $id_order_state, $id_supplier){        
        $sql = "SELECT id_order_detail from "._DB_PREFIX_."order_detail kod 
                    LEFT JOIN "._DB_PREFIX_."split_details ksd on ksd.root = kod.id_order
                    where ksd.id_order=".$id_order;
        ...
        $sql ='SELECT det.root as root, koh.id_order_state,COUNT(koh.id_order_state) FROM '._DB_PREFIX_.'split_details ksd
                LEFT JOIN '._DB_PREFIX_.'split_details det ON ksd.root = det.root
                LEFT JOIN '._DB_PREFIX_.'order_history koh ON koh.id_order=det.id_order
                WHERE koh.id_order_history IN(SELECT MAX(id_order_history) FROM '._DB_PREFIX_.'order_history WHERE id_order=koh.id_order AND id_order_state NOT IN('.OrderState::GENERATE_INVOICE.'))
                AND ksd.id_order='.$id_order.'
                GROUP BY koh.id_order_state';
        $checkParentOrderStatus = Db::getInstance()->executes($sql);
        $v = 38; //Order Complete
        $i_o_s = 0;
        foreach ($checkParentOrderStatus as $cpo) {
            if($v > $score[$cpo['id_order_state']]){
                $v = $score[$cpo['id_order_state']];
                $i_o_s = $cpo['id_order_state'];
            }
        }

        $parent_order = $checkParentOrderStatus[0]['root'];
        $order = new Order($parent_order);
        if($i_o_s != 22 && $i_o_s != 25){
            $history_new = new OrderHistory();
            $history_new->id_order = $parent_order;
            $history_new->id_employee = $id_supplier;
            $history_new->changeIdOrderState($i_o_s,$order);    
            $save = $history_new->save();
        }
        return true;
    }
}
