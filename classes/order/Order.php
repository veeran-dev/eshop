...
class OrderCore extends ObjectModel
{
    ...
    */
    public $round_type;
    /**id fulfillment center added by veeran.b**/
    public $id_fc;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_fc' =>                      array('type' => self::TYPE_INT),/**done by veeran.b**/
            'credit_days' =>                      array('type' => self::TYPE_INT),/**done by veeran.b**/
            'date_add' =>                    array('type' => self::TYPE_DATE, 'validate' => 'isDate'),
            'date_upd' =>                    array('type' => self::TYPE_DATE, 'validate' => 'isDate'),
            'estimated_delivery_time' =>    array('type' => self::TYPE_DATE),
            'id_fc' =>                    array('type' => self::TYPE_INT),
            'release_order' =>                    array('type' => self::TYPE_INT),
            'po_number' =>            array('type' => self::TYPE_STRING),
    ...
    */
    ...
    {
        $currentStateTab = OrderHistory::getLastOrderState($this->id);
        return Db::getInstance()->getRow('
			...
			WHERE osl.`id_lang` = '.(int)$id_lang.' AND os.`id_order_state` = '.(int)$currentStateTab->id);
    }
    ...
    {
        return count($this->getHistory((int)($this->id_lang), Configuration::get('PS_OS_PAR_SHIPPING')));
    }
     ...
     */
    public static function getCustomerOrders($id_customer, $show_hidden_status = false, Context $context = null,$limit = NULL)
    {
        if (!$context) {
    ...
    public static function setLastInvoiceNumber($order_invoice_id, $id_shop)
    {
        ...
        {
            $number = $cookie->consolidated_invoice;
        }
        if ($number) {
        ...
        if ($number) {
            ...
            // $sql .= '(SELECT new_number FROM (SELECT (MAX(koi.`number`) + 1) AS new_number
            // FROM `'._DB_PREFIX_.'order_invoice` koi
            // LEFT JOIN kob_orders ko on ko.id_order = koi.id_order 
            //     WHERE ko.id_fc = (SELECT b.id_fc FROM `kob_order_invoice` a 
            //                     LEFT JOIN kob_orders b on a.id_order = b.id_order  
            //                     WHERE a.id_order_invoice='.(int)$order_invoice_id.')
            // '.(Configuration::get('PS_INVOICE_RESET') ?
    ...
    public function getCurrentOrderState()
    {
        if ($this->current_state) {
            return OrderHistory::getLastOrderState($this->id);
