...
class OrderDetailCore extends ObjectModel
{
    ...
    /*** added by veeran.b ***/
    public $product_quantity_delivered;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'product_quantity_delivered' =>array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
    ...
    public function deleteItem($product_quantity, $unit_price_tax_excl)
    {
        $result_add = Db::getInstance()->insert('order_detail_bckup', array(
                    'id_order_detail' =>            (int)$this->id_order_detail,
                    'id_order' =>            (int)$this->id_order,
                    'product_id' =>            (int)$this->product_id,
                    'product_quantity' =>                (int)$product_quantity,
                    'unit_price_tax_excl' =>                $unit_price_tax_excl,
                    'id_tax_rules_group' =>            (int)$this->id_tax_rules_group,
    ...
    */
    ...
    public static function getDeliveredQuantity($id_order,$id_product){

        $query='SELECT kod.`product_quantity_delivered` FROM `kob_order_detail` kod 
                WHERE kod.`id_order`= '.$id_order.'
                AND kod.`product_id`='.$id_product;
        return Db::getInstance()->executeS($query);
    }
    public function updateQtyDelivered($qty)
    {
        $sql = 'UPDATE `'._DB_PREFIX_.'order_detail`
            SET `product_quantity_delivered` = '.(int)($qty).'
            WHERE `id_order_detail`= '.(int)($this->id_order_detail).'
            LIMIT 1';   
        return Db::getInstance()->Execute($sql);
    }
}
