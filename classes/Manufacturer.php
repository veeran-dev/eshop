...
class ManufacturerCore extends ObjectModel
{
    ...
    public static function getManufacturers($get_nb_products = false, $id_lang = 0, $active = true, $p = false, $n = false, $all_group = false, $group_by = false)
    {
		...
		SELECT m.*, ml.`description`, ml.`short_description`, m.id_manufacturer as value, m.name as label
	...
	public static function getProductsByCategory($id_manufacturer, $id_lang, $p, $n, $order_by = NULL, $order_way = NULL,
        $get_total = false, $active = true, $active_category = true, Context $context = NULL, $id_category = 'all')
    {
        if (!$context) {
            $context = Context::getContext();
        }
		...
		if 	($id_category != 'all'){ $category_filter = 'AND p.id_category_default = '.$id_category;} 
        ...
        if ($get_total) {
       		$sql = 	'SELECT p.`id_product`
					FROM `'._DB_PREFIX_.'product` p
					'.Shop::addSqlAssociation('product', 'p').'
					WHERE p.id_manufacturer = '.(int)$id_manufacturer. ' ' .$category_filter
					.($active ? ' AND product_shop.`active` = 1' : '').'
					'.($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '').'
					AND EXISTS (
						SELECT 1
