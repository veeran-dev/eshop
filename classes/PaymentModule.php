...
abstract class PaymentModuleCore extends Module
{
    ...
    public function validateOrder($id_cart, $id_order_state, $amount_paid, $payment_method = 'Unknown',
        ...
        $secure_key = false,$dash=0, Shop $shop = null, $split_order = false)	
		
    {
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            // if($order_reference_epaylater != null)
            //     $this->currentOrderReference = $order_reference_epaylater;
            // else
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
						...
						$productsList .=
						'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#FFFFFF').';">
                    ...
                    foreach ($cart_rules as $cart_rule) {
                        ...
                        if (count($order_list) == 1 && $values['tax_incl'] > ($order->total_products_wt - $total_reduction_value_ti) && $cart_rule['obj']->partial_use == 1 && $cart_rule['obj']->reduction_amount > 0) {
                            ...
                            if ($voucher->add()) {
                                ...
                                $notificationData = array('id_customer' => $this->context->customer->id_customer, 
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
						...
						if($customer->id_buyer == 3)
						{
							...
							if($this->context->cookie->payment_type != 5) { // If selected payment option is not approval pending  
                                ...
                                $notificationData = array('id_customer' => $this->context->cookie->id_customer, 
                                                          'notification_type' => $notifyType->id,
                                                          'url' => $notifyType->url."/".$order->id,
                                                          'content' => $em_sub);
                                Notifications::addNotification($notificationData);
                                
                                Mail::Send((int)$order->id_lang, 'order_conf', Mail::l($em_sub, (int)$order->id_lang), $data, $this->context->customer->email, $this->context->customer->firstname.' '.$this->context->customer->lastname, NULL, NULL, $fileAttachment,null, _PS_MAIL_DIR_, false, (int)$order->id_shop);
							}
							else
							{
								if(!$split_order) {
                                    $em_sub = 'Order confirmation | The Order with Kobster.com [#'. $order->id .'] is pending for approval';
                                
                                    $notifyType = new Notifications(2);
                                    $notificationData = array('id_customer' => $customer->id, 
                                                              'notification_type' => $notifyType->id,
                                                              'url' => $notifyType->url."/".$order->id,
                                                              'content' => $em_sub);
                                    Notifications::addNotification($notificationData);
                                    ...
                                    Mail::Send((int)$order->id_lang, 'order_pending_approval', Mail::l($em_sub, (int)$order->id_lang), $data, $customer->email, $customer->firstname.' '.$customer->lastname, NULL, NULL, $fileAttachment,null, _PS_MAIL_DIR_, false, (int)$order->id_shop);
                                    
                                    //Send email to approver
                                    $approver = $customer->getApprover(false, true);
                                    
                                    for($i = 0 ; $i < sizeof($approver); $i++) {
                                        $approver_details = new Customer($approver[$i]['id_parent']);
                                        
                                        $notifyType = new Notifications(2);
                                        $notificationData = array('id_customer' => $approver_details->id, 
                                                              'notification_type' => $notifyType->id,
                                                              'url' => $notifyType->url."/".$order->id,
                                                              'content' => $em_sub);
                                        ...
                                        Mail::Send((int)$order->id_lang, 'order_submit_approval', Mail::l($em_sub, (int)$order->id_lang), $data, $approver_details->email, $approver_details->firstname.' '.$approver_details->lastname, NULL, NULL, $fileAttachment);

                                        if($approver_details->mobile) {
                                            $msgtxt='Hello '.$approver_details->firstname.' , a new order # '.$order->id.' is pending for your approval. - Kobster.com';
                                            SMSAlert::sendSMSAlert($approver_details->mobile, $msgtxt);
                                        }
                                    }

                                    if((int)$customer->getCustomerRole() == 1 || (int)$customer->getCustomerRole() == 2 && $this->context->cookie->payment_type == 5 && $this->context->cookie->creator_customer_id == 0 ) 
                                    {
                                        $old_orgin_order = Order::getOrginOrderId($order->id);
                                        if($old_orgin_order == "")
                                             Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('INSERT INTO '._DB_PREFIX_.'orgin_orders (`id_old_order`, `id_new_order`, `id_customer`) VALUES ('.$order->id.','.$order->id.','.$this->context->cookie->id_customer.')');
							...
							if($order->estimated_delivery_time != '0000-00-00 00:00:00')
								$estimated_time = ' You can expect delivery on '.date("F j, Y, g:i a",strtotime($order->estimated_delivery_time)).'';

							// check whether the approver revise the creator(child) order .
							
							if($this->context->cookie->creator_customer_id != 0)
							{
								//change the order->id_customer to child id  
								$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('UPDATE '._DB_PREFIX_.'orders set id_customer = '.$this->context->cookie->creator_customer_id.' where id_order = '.$order->id.'');
								
								/*Change loyalty point to 0 for cancelled order*/
								 Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('UPDATE '._DB_PREFIX_.'loyalty set points = 0 where id_order = '.$this->context->cookie->creator_old_order_id.'');
								 
								 /*Change loyalty id_customer to child_id_customer for newly placed order*/
								 Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('UPDATE '._DB_PREFIX_.'loyalty set id_customer = '.$this->context->cookie->creator_customer_id.' where id_order = '.$order->id.'');
								 
								
								 
								 /*Check for old orgin order id*/
								 $old_orgin_order = Order::getOrginOrderId($creator_old_order_id);
								
								if($old_orgin_order != "")
								//
								{
									/*insert the old order id with new order id(revised order id) in kob_orgin_order for future purpose*/
									Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('INSERT INTO '._DB_PREFIX_.'orgin_orders (`id_old_order`, `id_new_order`, `id_customer`) VALUES ('.$old_orgin_order.','.$order->id.','.$this->context->cookie->id_customer.')');
									
									$orgin_child_orders = Order::getOrginChildIds($old_orgin_order);
									for($i=0; sizeof($orgin_child_orders)>$i; $i++ )
									{
										if($orgin_child_orders[$i]['id_customer'] != $this->context->cookie->id_customer)
										{
											
											$old_child = new Customer((int)($orgin_child_orders[$i]['id_customer']));
											
											$em_sub = 'Order confirmation | Your Order with Kobster.com [#'. $order->id .'] has  been successfully placed!';

                                            $notifyType = new Notifications(1);
                                            $notificationData = array('id_customer' => $old_child->id, 
                                                          'notification_type' => $notifyType->id,
                                                          'url' => $notifyType->url."/".$order->id,
                                                          'content' => $em_sub);
                                            Notifications::addNotification($notificationData);
											
											Mail::Send(1, 'revise_order_conf', Mail::l($em_sub, 1), $data, $old_child->email, $old_child->firstname.' '.$old_child->lastname, NULL, NULL, $fileAttachment);
								...
								else
								{
                                    ...
                                    $notifyType = new Notifications(1);
                                    $notificationData = array('id_customer' => $child->id, 
                                                          'notification_type' => $notifyType->id,
                                                          'url' => $notifyType->url."/".$order->id,
                                                          'content' => $em_sub);
                                    Notifications::addNotification($notificationData);
									
                                    Mail::Send((int)$order->id_lang, 'revise_order_conf', Mail::l($em_sub, (int)$order->id_lang), $data, $child->email, $child->firstname.' '.$child->lastname, NULL, NULL, $fileAttachment);
								}

                                /**
                                  * Below block is used for order revising purpose and reset older datas
                                  * After placing new order -> old order should be revised and cancelled
                                **/
								if($result == 1){
                                    $orderObj = new Order((int)($this->context->cookie->creator_old_order_id));
                                    $orderObj->reviseChildOrder($orderObj->id_customer);
									$this->context->cookie->creator_customer_id=0;	
									$this->context->cookie->creator_old_order_id=0;
								}
							}
						}
                        elseif($customer->id_buyer == 10)
                        {
                            $em_sub = 'Order confirmation | Your Order with Kobster.com [#'. $order->id .'] has  been successfully placed!';
                            Mail::Send((int)$order->id_lang, 'perks_order_conf', Mail::l($em_sub, (int)$order->id_lang), $data, $this->context->customer->email, $this->context->customer->firstname.' '.$this->context->customer->lastname, NULL, NULL, $fileAttachment,null, _PS_MAIL_DIR_, false, (int)$order->id_shop);   
                        }
						else
						{
                        ...
                        if($this->context->cookie->payment_type == 5)
                        {
                            ...
                            if($customer->mobile) {
                                $msgtxt = 'Hello '.$customer_name.', Your order # '.$order->id.' has been sent for approval - Kobster.com';
                            ...
                            if($customer->mobile){
                                $msgtxt='Hello '.$customer->firstname.' '.$customer->lastname.' , Your order # '.$order->id.' has been placed successfully.'.$estimated_time.' - Kobster.com';
                                SMSAlert::sendSMSAlert($customer->mobile, $msgtxt);
                            }
                            elseif($invoice->phone_mobile){
                                $msgtxt='Hello '.$customer->firstname.' '.$customer->lastname.' , Your order # '.$order->id.' has been placed successfully.'.$estimated_time.' - Kobster.com';
