...
class GroupCore extends ObjectModel
{
    ...
    /*** credit days added by veeran.b ***/
    public $credit_days;

    /** @var string verification document */
    public $verification_document;

    /** @var string verification status */
    public $verification_status;

    /** @var int po_mandatory */
    public $po_mandatory;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'credit_days' =>    array('type' => self::TYPE_INT),
            'verification_document' =>    array('type' => self::TYPE_STRING),
            'verification_status' =>    array('type' => self::TYPE_INT),
            'po_mandatory' => array('type' => self::TYPE_INT),
            'id_kam' => array('type' => self::TYPE_INT),
    ...
    public static function getGroups($id_lang, $id_shop = false)
    {
		...
		SELECT DISTINCT g.`id_group`, g.`reduction`,g.`verification_document`,g.`verification_status`, g.`po_mandatory`, g.`price_display_method`, gl.`name`
    ...
    public function getCustomers($count = false, $start = 0, $limit = 0, $shop_filtering = false)
    {
        if ($count) {
            ...
            LEFT JOIN `'._DB_PREFIX_.'customer_role` cl ON (c.`id_customer` = cl.`id_customer`)
			WHERE cg.`id_group` = '.(int)$this->id.'
			'.($shop_filtering ? Shop::addSqlRestriction(Shop::SHARE_CUSTOMER) : '').'
			AND c.`deleted` != 1');
        }
        return Db::getInstance()->executeS('
		SELECT cg.`id_customer`, c.*,cl.`id_role` as customer_role
		FROM `'._DB_PREFIX_.'customer_group` cg
		LEFT JOIN `'._DB_PREFIX_.'customer` c ON (cg.`id_customer` = c.`id_customer`)
        LEFT JOIN `'._DB_PREFIX_.'customer_role` cl ON (c.`id_customer` = cl.`id_customer`)
		WHERE cg.`id_group` = '.(int)$this->id.'
    ...
    {
        $result = Db::getInstance()->ExecuteS('SELECT c.`id_customer` FROM `'._DB_PREFIX_.'customer` c
                                            WHERE c.`id_default_group` = '.$this->id.'');
