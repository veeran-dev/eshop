...
class AdminProductsControllerCore extends AdminController
{
    ...
    protected function copyFromPost(&$object, $table)
    {
        ...
        if ($this->isTabSubmitted('Informations')) {
            ...
            if ($this->checkMultishopBox('discontinued', $this->context)) {
                $object->discontinued = (int)Tools::getValue('discontinued');
            }

            if(Tools::getIsset('active') && !(int)Tools::getValue('active')) {
                $catalogHistory = new CatalogHistory();
                $catalogHistory->addHistory("status", $object->id, $content['0'] = array('from' => 'enable', 'to' => 'disable'));
            }
    ...
    public function rmAlertMail($id_product,$product_name,$reference)
    {
        global $cookie;
        ...
        $customer_details='';       
        $customer_Id=RateContract::rmAlertDisabledProduct($id_product);
        
        for($i=0;$i<sizeof($customer_Id);$i++)
        {
            if(($customer_Id[$i]['id_employee']==$customer_Id[$i+1]['id_employee']) || ($customer_Id[$i]['id_employee']==$customer_Id[$i-1]['id_employee']) || ($customer_Id[$i+1]['id_employee']=='')) 
            {
                $customer_details .='<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
                                    <td style="width:20%; padding: 0.6em 0;">'.($i+1).'</td>
                                    <td style="width:35%; padding: 0.6em 0;"><strong>'.$customer_Id[$i]['firstname'].'</strong></td>
                                    <td style="width:45%; padding: 0.6em 0;">'.$customer_Id[$i]['customer_email'].'</td>            
                                    </tr>';             
            }
            $j=$i+1;
            if(($customer_Id[$i]['id_employee']!=$customer_Id[$j]['id_employee']))
            {
                $data = array(
                    '{firstname}' => 'Relationship Manager',
                    '{emp_name}'=>$emp_name,
                    '{product_name}' => $product_name,
                    '{product_id}' => $id_product,
                    '{product_ref}' => $reference,
                    '{details}' => $customer_details);
                $em_sub = 'Product Disabled/Discontinued ';
                /*Mail::Send((int)$cookie->id_lang, 'productdisabled', Mail::l($em_sub, (int)$cookie->id_lang), $data, $customer_Id[$i]['email'], 'Relationship Manager', NULL, NULL, NULL);*/
                Mail::Send((int)$cookie->id_lang, 'productdisabled', Mail::l($em_sub, (int)$cookie->id_lang), $data, 'price.change@kobster.com', 'Relationship Manager', NULL, NULL, NULL);
            }
    ...
    public function processPriceAddition()
    {
        ...
        if (($price == '-1') && ((float)$reduction == '0')) {
            ...
            $this->errors[] = Tools::displayError('Submitted reduction value (0-100) is out-of-range');
        } elseif ($price != '-1' && $product->price < $price) {
            $this->errors[] = Tools::displayError('Specific price exceeds MRP of this product.');
    ...
    public function initProcess()
    {
        if (Tools::isSubmit('submitAddproductAndStay') || Tools::isSubmit('submitAddproduct')) {
            ...
            $object = new Product($this->id_object);
            
            if($this->isTabSubmitted('Informations')){

                $product_name = $this->getFieldValue($object, 'name_1');
                
                if($object->name['1'] != $product_name)
                {
                    $content['0'] = array(
                        'from' => $object->name['1'],
                        'to' => $product_name
                    ...
                    $catalogHistory = new CatalogHistory();
                    $catalogHistory->addHistory("name", Tools::getValue('id_product'), $content);
                }
                if($object->discontinued != (int)Tools::getValue('discontinued'))
                {
                    if(Tools::getValue('discontinued') == 1){
                        $content['0'] = array(
                                'from' => 0,
                                'to' => 1
                                );
                        $catalogHistory = new CatalogHistory();
                        $catalogHistory->addHistory("Discontinued", Tools::getValue('id_product'), $content);
                    }
                }

                
                
                $product_zones = Tools::getValue('zone');
                
                if(isset($product_zones)) {
                    Product::insertUpdateZone($this->id_object, $product_zones);
                }
            }
            ...
            if($this->isTabSubmitted('Quantities')){
                $moq = $product_name = $this->getFieldValue($object, 'minimal_quantity');
                if($object->minimal_quantity != $moq)
                {
                    $content['0'] = array(
                        'from' => $object->minimal_quantity,
                        'to' => $moq
                        );
                    $catalogHistory = new CatalogHistory();
                    $catalogHistory->addHistory("minimal_quantity", Tools::getValue('id_product'), $content);
                }
            }
            if ($this->isTabSubmitted('Prices')) {
                ...
                {
                    $vat =0;
                    $new_price = $this->getFieldValue($object, 'price');
                    $id_tax_rules_group = $this->getFieldValue($object, 'id_tax_rules_group');
                    if($object->id_tax_rules_group !=$this->getFieldValue($object, 'id_tax_rules_group'))
                    {
                        $newTax = new TaxRulesGroup($id_tax_rules_group);
                        $oldTax = new TaxRulesGroup($object->id_tax_rules_group);
                        $content['0'] = array(
                            'from' => $oldTax->name,
                            'to' => $newTax->name
                            );
                        $catalogHistory = new CatalogHistory();
                        $catalogHistory->addHistory("Tax group", Tools::getValue('id_product'), $content);

                        $content_new=array("id_product" => Tools::getValue('id_product'), "old_vat" => $object->id_tax_rules_group, "new_vat"=>$this->getFieldValue($object, 'id_tax_rules_group'),"id_employee" => $this->context->employee->id);
                        priceChange::VATChangeNotification($content_new);
                        $vat = 1;
                    }

                    $wholesale_price = $this->getFieldValue($object, 'wholesale_price');

                    $validate_prices = $this->object->validatePrice($new_price, Tools::getValue('wholesale_price'));
                    
                    if(is_array($validate_prices) && !empty($validate_prices)) {
                       foreach ($validate_prices as $errors) {
                            $this->displayInformation($errors);   
                       }
                    }

                    if(!count($this->errors) && $object->wholesale_price != $wholesale_price) {
                        $content['0'] = array('from' => $object->wholesale_price, 'to' => $wholesale_price);
                        $catalogHistory = new CatalogHistory();
                        $catalogHistory->addHistory("wholesale_price", Tools::getValue('id_product'), $content);
                    }

                    if($object->price != $new_price && $vat != 1)
                    {
                        //Product::saveOldPrice(Tools::getValue('id_product'),$object->price, $new_price,0,$this->context->employee->id);
                        $priceHistory = new PriceHistory();
                        $priceHistory->id_product = Tools::getValue('id_product');
                        $priceHistory->id_group = 0;
                        $priceHistory->new_price = $new_price;
                        $priceHistory->old_price = $object->price;
                        $priceHistory->id_employee = $this->context->employee->id;
                        $priceHistory->add();
                        
                        $content_new=array("id_product" => Tools::getValue('id_product'), "old_price_spec" => $object->price, "new_price"=>$new_price,"id_employee" => $this->context->employee->id);
                        priceChange::priceChangedNotification($content_new);
                    }
                }
            }
        }
    ...
    public function checkProduct()
    {
        ...
        if($this->isProductFieldUpdated('price_displayed')) {
            if($this->isProductFieldUpdated('wholesale_price')) {
                if(!$id_product = Tools::getValue('id_product')) {
                    $ws_price = Tools::getValue('wholesale_price');
                    $te_price = Tools::getValue('price_displayed');
                    if($ws_price && floatval($ws_price) > floatval($te_price)) {
                        $this->errors[] = $this->l('This product wholsale price exceeds MRP.');
                    }
                }
            }
        }
    ...
    */
    ...
    public function initFormInformations($product)
    {
        ...
        $productZones=$product->getProductZones($product->id);
        $zones = FulfillmentCentre::getAllFCentres();
        $zone_array = array();
        foreach($productZones as $zone){
            $zone_array[] = $zone['zone_id'];
        }

        $available_across_india = 0;

        foreach($zones as &$zone) {
            $zone['available'] = (in_array($zone['id_fulfillment_centre'], $zone_array) ? 1 : 0);
        }

        if(in_array(0, $zone_array))
            $available_across_india = 1;
        ...
        $data->assign(array(
            'languages' => $this->_languages,
            'zones' => $zones,
            'available_across_india' => $available_across_india,
            'default_form_language' => $this->default_form_language,
            'currency' => $currency
        ));
        $this->object = $product;
        ...
        */
        ...
        $product_props = array();
        // global informations
        array_push($product_props, 'reference', 'ean13', 'upc','discontinued',
    ...
    public function ajaxProcessaddProductImage()
    {
        ...
        $logger->setFilename('testimage.txt');
        $logger->logError('');
