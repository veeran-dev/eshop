                ...
                if (($error = $cm->validateField('message', $cm->message, null, array(), true)) !== true) {
                    ...
                    if($cm->private==1)
                    {
                      /* Email alert to the customer support & supply chain team */                            
                        ...
                        $varsTpl = array(
                            '{id_order}' => $customer_message->id_order, 
                            '{emp_name}' => $employee->firstname, 
                            '{message}' => (Configuration::get('PS_MAIL_TYPE') == 2 ? $customer_message->message : nl2br2($customer_message->message)),
                            '{company}' => $group->name
                        );
                        ...
                        if($customer->id_relationship_manager > 0)
                        {
                            @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                Mail::l('Order Comment - #'. $order->id .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, $relationship_manager->email,
                                'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                        }
                        else
                        {
                            @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                Mail::l('Order Comment - #'. $order->id .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, 'support@kobster.com',
                                'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                        ...
                        {
                            @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                Mail::l('Order Comment - #'. $order->id .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, $fc_head->email,
                                'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
                        }

                        @Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                Mail::l('Order Comment - #'. $order->id .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, 'ops@kobster.com',
                                'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
						@Mail::Send((int)($order->id_lang), 'order_internal_comment',
                                Mail::l('Order Comment - #'. $order->id .' by '. $employee->firstname, (int)($order->id_lang)), $varsTpl, 'lingakani.g@kobster.com',
                                'Team', NULL, NULL, NULL, NULL, _PS_MAIL_DIR_, true);
