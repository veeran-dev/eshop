...
class AdminCategoriesControllerCore extends AdminController
{
    ...
    public function renderForm()
    {
        ...
        $es_array = array();
        $suppliers = EliteSupplier::getAllSupplier();
        $es_array[0]['id_supplier'] = 0;
        $es_array[0]['company'] = '-';
        $z = 1;
        foreach ($suppliers as $s) {
            $es_array[$z]['id_supplier'] = $s['id_supplier'];
            $es_array[$z]['company'] = $s['company'];
            $z++;
        }
    ...
    {
        if(Tools::isSubmit('submitAdd'.$this->table) && Tools::getValue('tag_supplier') != 0){
            EliteSupplierProducts::tagCategory(Tools::getValue('id_category'), Tools::getValue('tag_supplier'));
        }
