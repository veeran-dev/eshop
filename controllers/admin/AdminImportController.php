...
class AdminImportControllerCore extends AdminController
{
    ...
    public static $defualt_zones = array();
    ...
    public static $validators = array(
        ...
        /* only brands brands_mrp */
        'brands_mrp' => array('AdminImportController', 'getPrice'),
        'chennai_price' => array('AdminImportController', 'getPrice'),
        'Bengaluru_price' => array('AdminImportController', 'getPrice'),
        'Delhi_price' => array('AdminImportController', 'getPrice'),
        'Mumbai_price' => array('AdminImportController', 'getPrice'),
        'Hyderabad_price' => array('AdminImportController', 'getPrice'),
        /** Tax included */
        ...
        'hsn_code' => array('AdminImportController', 'getHsn')
    );
        ...
        switch ((int)Tools::getValue('entity')) {
            ...
            case $this->entities[$this->l('Products')]:
                ...
                $this->available_fields = array(
                    ...
                    'brands_mrp' => array('label' => $this->l('Brands MRP')),
                    'chennai_price' => array('label' => $this->l('Chennai MRP')),
                    'bengaluru_price' => array('label' => $this->l('Bengaluru MRP')),
                    'mumbai_price' => array('label' => $this->l('Mumbai MRP')),
                    'delhi_price' => array('label' => $this->l('Delhi MRP')),
                    'hyderabad_price' => array('label' => $this->l('Hyderabad MRP')),
                    ...
                    'reference' => array('label' => $this->l('Reference #')),
                    'hsn_code' => array('label' => $this->l('HSN Code #')),
                    'supplier_reference' => array('label' => $this->l('Supplier reference #')),
                    ...
                    'zones' => array(
                        'label' => $this->l('Zones (x,y,z...)'),
                        'help' => $this->l('Products zones. 0 - ALL, 1 - Bengaluru, 2 - Chennai, 3 - Hyderabad, 4 - Mumbai, 6 - New Delhi')
                    )
                );
            ...
            case $this->entities[$this->l('Inventory')]:
                $this->available_fields = array(
                    'no' => array('label' => $this->l('Ignore this column')),
                    'id_product' => array('label' => $this->l('id_product')),
                    'id_fc' => array('label' => $this->l('id_fc')),
                    'quantity' => array('label' => $this->l('Quantity')),
                    'added' => array('label' => $this->l('added')),
                    'removed' => array('label' => $this->l('removed')),
                    'date' => array('label' => $this->l('date')),
                );
            break;
            case $this->entities[$this->l('TagSupplier')]:
                $this->available_fields = array(
                    'no' => array('label' => $this->l('Ignore this column')),
                    'id_product' => array('label' => $this->l('id_product')),
                    'id_supplier' => array('label' => $this->l('id_supplier')),
                    'id_zone' => array('label' => $this->l('id_zone')),
                    'moq' => array('label' => $this->l('moq')),
                    'oos' => array('label' => $this->l('oos')),
                );
            break;
            ...
            case $this->entities[$this->l('Addresses')]:
                ...
                $this->available_fields = array(
                    ...
                    'vat_number' => array('label' => $this->l('GST number')),
    ...
    public function renderForm()
    {
        ...
        $this->tpl_form_vars = array(
            ...
            'tag_supplier_confirmation' => Tools::isSubmit('validateTagSupplierImportFile') && (isset($this->warnings) && !count($this->warnings) && !count($this->errors)),
    ...
    public function renderView()
    {
        ...
        $valid = CsvImport::checkValidFile(Tools::getValue('csv'));
        $pin = CsvImport::getPin(Tools::getValue('csv'));
        $file_name = Tools::getValue('csv');
        $employee_data=Employee::getEmployeesByProfile(7);
        $inventory_submit = Tools::getValue('entity') == 8 ? true: false;
        $tag_supplier = Tools::getValue('entity') == 9 ? true: false;
        $this->context->smarty->assign(array(
            'valid' => $valid['valid'],
            'imported' => $valid['imported_by'],
            'file_name' => $file_name,
            'pin' => $pin['password'],
    ...
    protected static function getHsn($field) 
    {
        $field_value = "";

        $length_of_field = strlen($field);

        if($length_of_field > 3 && $length_of_field <= 8) {
            return $field_value = $field;
        }
        return $field_value;
    }
    ...
    */
    ...
    public function inventoryImport(){
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++) {
            ...
            // if($product->active != 1){
            //     $this->errors[] = (Tools::displayError('Please check product is disabled'));
            // }
            
            // if(!($info['added'] == "0" || $info['added'] == "1")){
            //     $this->errors[] = (Tools::displayError('Please check added field, negative value not allowed'));
            //     $save = 0;
            // }

            // if(!($info['removed'] == "0" || $info['removed'] == "1")){
            //     $this->errors[] = (Tools::displayError('Please check removed field, negative value not allowed'));
            //     $save = 0;
            // }
            
            if(($info['added'] == $info['removed']) || ($info['added'] >1) || ($info['removed'] >1)){
                $this->errors[] = (Tools::displayError($current_line.' : Please check added / removed field'));   
                $save = 0;
            }

            if($info['added'] < 0){
                $this->errors[] = (Tools::displayError($current_line.' : Please check added field, negative value not allowed'));   
                $save = 0;
            }

            if($info['removed'] < 0){
                $this->errors[] = (Tools::displayError($current_line.' : Please check removed field, negative value not allowed'));   
                $save = 0;
            }
            
            if($info['quantity'] <= 0){
                $this->errors[] = (Tools::displayError($current_line.' : Please check quantity field'));   
                $save = 0;
            }

            if(!FulfillmentCentre::hasFc($info['id_fc'])){
                $this->errors[] = (Tools::displayError($current_line.' : Please check id_fc'));   
                $save = 0;
            }

            if(!Product::isValidProduct($info['id_product'])){
                $this->errors[] = (Tools::displayError($current_line.' : Please check id_product'));   
                $save = 0;
            }

            $current_date = new DateTime();
            if(strtotime(date_format($date,"Y-m-d H:i:s")) > strtotime($current_date->format("Y-m-d H:i:s"))){
                $this->errors[] = (Tools::displayError($current_line.' : Please check date field'));      
                $save = 0;
            }
            if($save == 1){
                $inventory = new Inventory();
                $inventory->id_product =$info['id_product'] ;
                $inventory->id_fulfillment_centre = $info['id_fc'] ;
                $inventory->quantity = $info['quantity'] ;
                $inventory->added = $info['added'] ;
                $inventory->removed = $info['removed'] ;
                $inventory->date = date_format($date,"Y-m-d H:i:s");
                $inventory->save();
            }
        }
    }
    ...
    public function productImport()
    {
        ...
        Module::setBatchMode(true);
         ...
         $taxChanges = array();
        $nameChanges = array();
        $wholesaleChanges = array();
        $minimalChanges = array();
        $stockChanges = array();
        $discontinuedChanges= array();
        
        $taxProducts = array();
        $nameProducts = array();
        $wholesaleProducts= array();
        $minimalProducts = array();
        $stockProducts = array();
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++) {
            ...
            if($info['id']){
                if(!empty($info['id_tax_rules_group']) && ($product->id_tax_rules_group != $info['id_tax_rules_group']))
                {
                    array_push($taxProducts, $product->id);
                    //$taxProducts[$a] = $product->id;
                    $taxChanges[$a] = array(
                            'from' => TaxRulesGroup::getTaxesName($product->id_tax_rules_group),
                            'to' => TaxRulesGroup::getTaxesName($info['id_tax_rules_group']),
                            );
                    $a++;
                }
                if(!empty($info['name']) && ($product->name['1'] != $info['name']))
                {
                    array_push($nameProducts, $product->id);
                    //$nameProducts[$b] = $product->id;
                    $nameChanges[$b] = array(
                            'from' => $product->name['1'],
                            'to' => $info['name']
                            );
                ...
                if(!empty($info['minimal_quantity']) && ($product->minimal_quantity != $info['minimal_quantity']))
                {
                    array_push($minimalProducts, $product->id);
                    //$minimalProducts[$d] = $product->id;
                    $minimalChanges[$d] = array(
                            'from' => $product->minimal_quantity,
                            'to' => $info['minimal_quantity']
                            );
                    $d++;
                }
                if(!empty($info['wholesale_price']) && ($product->wholesale_price != $info['wholesale_price']))
                {
                    array_push($wholesaleProducts, $product->id);
                    //$wholesaleProducts[$e] = $product->id;
                    $wholesaleChanges[$e] = array(
                            'from' => $product->wholesale_price,
                            'to' => $info['wholesale_price']
                            );
                    $e++;
                }

                if(!empty($info['out_of_stock']) && ($product->out_of_stock != $info['out_of_stock']))
                {
                    array_push($stockProducts, $product->id);
                    //$wholesaleProducts[$e] = $product->id;
                    $stockChanges[$f] = array(
                            'from' => $product->out_of_stock,
                            'to' => $info['out_of_stock']
                            );
                    $f++;
                }

                if(!empty($info['discontinued']) && ($product->discontinued != $info['discontinued']))
                {
                    array_push($discontinuedProducts, $product->id);
                    //$wholesaleProducts[$e] = $product->id;
                    $discontinuedChanges[$g] = array(
                            'from' => $product->discontinued,
                            'to' => $info['discontinued']
                            );
                    $g++;
                }
            }
            ...
            if ((int)$product->id_tax_rules_group != 0) {
                if (Validate::isLoadedObject(new TaxRulesGroup($product->id_tax_rules_group))) {
                    ...
                    $product->tax_rate=TaxRulesGroup::getTaxesRate($product->id_tax_rules_group,110,0);
                } else {
            ...
            // Update HSN code for a product
            if(isset($product->hsn_code) && $product->hsn_code != "")
                $product->hsn_code = $product->hsn_code;
            
            if(isset($product->brands_mrp))
            {
                $product->mrp = $product->brands_mrp;
            }
            ...
            if ($field_error === true && $lang_field_error === true) {
                ...
                if (!$res) {
                    if (isset($product->date_add) && $product->date_add != '') {
                        $ref = (int)(Configuration::get('PS_PRODUCT_REFERENCE'));
                        Configuration::updateValue('PS_PRODUCT_REFERENCE', $ref + 1);
                ...
                if($res) {
                    ...
                    if(isset($product->chennai_price) && $product->chennai_price != "") {
                        $active = 1;
                        ZonalPrice::updatePriceByZone($product->id, 2, $product->chennai_price);
                    }

                    if(isset($product->mumbai_price) && $product->mumbai_price != "") {
                        $active = 1;
                        ZonalPrice::updatePriceByZone($product->id, 4, $product->mumbai_price);
                    }

                    if(isset($product->delhi_price) && $product->delhi_price != "") {
                        $active = 1;
                        ZonalPrice::updatePriceByZone($product->id, 6, $product->delhi_price);
                    }

                    if(isset($product->bengaluru_price) && $product->bengaluru_price != "") {
                        $active = 1;
                        ZonalPrice::updatePriceByZone($product->id, 1, $product->bengaluru_price);
                    }

                    if(isset($product->hyderabad_price) && $product->hyderabad_price != "") {
                        $active = 1;
                        ZonalPrice::updatePriceByZone($product->id, 3, $product->hyderabad_price);
                    } 

                    // Expiring rate contract prices if price goes down that quoted prices
                    $rate_contract_products = $product->getRateContractProducts();
                    if(!empty($rate_contract_products)) {
                        foreach ($rate_contract_products as $rc_product) {
                            ...
                            if($specific_price->price > $product->price) {
                                ...
                                if($specific_price->update()) {
                                    ...
                                    CatalogHistory::addHistory('rcp_expired', $product->id, $content);
                                }
                            }
                        }
                    } 
            ...
            if (!$res) {
                ...
                if ((isset($info['reduction_price']) && $info['reduction_price'] > 0) || (isset($info['reduction_percent']) && $info['reduction_percent'] > 0)) {
                    foreach ($id_shop_list as $id_shop) {
                        ...
                        if($specific_price->reduction_type == 'amount') {
                            if($specific_price->reduction > $product->price) {
                                $this->errors[] = Tools::displayError('Specific price exceeds MRP.');
                            }
                        }

                        if (empty($this->errors) && !$specific_price->save()) {
                ...
                if(isset($product->zones)) {
                    $zones = array();
                    $zones = explode($this->multiple_value_separator, $product->zones);
                    if(is_array($zones))
                        $product->insertUpdateZone($product->id, $zones);
                }
                 ...
                 * Check stock available quantity exists and update product quantity
                 */
                if($quantity_available = StockAvailable::getQuantityAvailableByProduct($product->id, $product->id_product_attribute)) {
                    $product->quantity = intval($quantity_available);
                }
        ...
        if(empty($this->errors))
        {
            if(!empty($taxChanges))
            {
                ...
                $catalogHistory->addHistory('Tax group',$taxProducts, $taxChanges);
            }
            if(!empty($nameChanges))
            {
                $catalogHistory = new CatalogHistory();
                $catalogHistory->addHistory('name',$nameProducts, $nameChanges);
            }

            if(!empty($wholesaleChanges))
            {
                $catalogHistory = new CatalogHistory();
                $catalogHistory->addHistory('wholesale_price',$wholesaleProducts, $wholesaleChanges);
            }
            if(!empty($minimalChanges))
            {
                $catalogHistory = new CatalogHistory();
                $catalogHistory->addHistory('minimal_quantity',$minimalProducts, $minimalChanges);
            }
            if(!empty($stockChanges))
            {
                $catalogHistory = new CatalogHistory();
                $catalogHistory->addHistory('Out of stock',$stockProducts, $stockChanges);
            }
            if(!empty($discontinuedChanges))
            {
                $catalogHistory = new CatalogHistory();
                $catalogHistory->addHistory('Discontinued',$discontinuedProducts, $discontinuedChanges);
            }
            $file_name=Tools::getValue('csv');
            $valid = CsvImport::checkValidFile($file_name);
            
            $csv = new CsvImport($valid['id_csv']);
            $csv->imported_by=$this->context->employee->id;
            $csv->date_upd=$now;
            $csv->save();
        }
        $this->closeCsvFile($handle);
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('validateImportFile') && (Tools::getValue('entity')==$this->entities[$import_type = $this->l('Products')])) {

            $this->entities[$import_type = $this->l('Products')];
            $this->validateProductImport();
            $this->clearSmartyCache();

        }
        else if (Tools::isSubmit('validateImportFile') && (Tools::getValue('entity')==$this->entities[$import_type = $this->l('Inventory')])) {

            $this->entities[$import_type = $this->l('Inventory')];
            $this->inventoryImport();
            $this->clearSmartyCache();

        }
        else if (Tools::isSubmit('validateTagSupplierImportFile') && (Tools::getValue('entity')==$this->entities[$import_type = $this->l('TagSupplier')])) {

            $this->entities[$import_type = $this->l('TagSupplier')];
            $this->tagSupplierImport();
            $this->clearSmartyCache();
            ...
            if (Tools::getValue('csv')) {
                ...
                switch ((int)Tools::getValue('entity')) {
                    ...
                    case $this->entities[$import_type = $this->l('Products')]:
                        ...
                        $this->clearSmartyCache();
                        break;
                    case $this->entities[$import_type = $this->l('Inventory')]:
                        $this->inventoryImport();
                        $this->clearSmartyCache();
                        break;
                    ...
                    case $this->entities[$import_type = $this->l('Alias')]:
                        $this->aliasImport();
                        break;
                    case $this->entities[$import_type = $this->l('TagSupplier')]:
                        $this->tagSupplierImport();
                        $this->clearSmartyCache();
                        break;
                }
