    ...
    public function postProcess()
    {
        ...
        elseif (Tools::isSubmit('uploadDRScan') AND ($id_order = (int)(Tools::getValue('id_order'))) AND Validate::isLoadedObject($order = new Order($id_order)))
        {
            ...
            elseif($_FILES["file"]["size"] > 2098576)
            {
              ...
              if(move_uploaded_file($_FILES["file"]["tmp_name"],""._PS_CORE_DIR_."/scanedDRs/".$delivery->generateFilename(). "." . $extension)){
                    
                    $delivery->dr_file_name = $delivery->generateFilename(). "." .$extension;
                    $delivery->update();
