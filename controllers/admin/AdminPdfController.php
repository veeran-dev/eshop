...
{  
    ...
    {
        $loggedInternal = Tools::getValue('loggedInternal');

        if(isset($loggedInternal) && $loggedInternal) {
            $this->logged_internal = true;
        }
    ...
    public function processGenerateConsolidatedInvoicePdf()
    {
        if (Tools::isSubmit('id_order')) {
            $this->generateConsolidatedInvoicePDFByIdOrder(Tools::getValue('id_order'));
        }
        else {
            die(Tools::displayError('The order ID -- or the invoice order ID -- is missing.'));
        }
    ...
    {
        if (Tools::isSubmit('id_delivery')) {
            $this->generateDeliverySlipPDFByIdDelivery((int)Tools::getValue('id_delivery'));
        }
        else{
    ...
    public function processGenerateDeliveryLabel()
    {
        $id_delivery_label = (int)Tools::getValue('id_delivery_label');
        $delivery = new Delivery($id_delivery_label);
        $orderDelivery = OrderDelivery::getByDeliveryId($id_delivery_label);
        $order = new Order((int)OrderDelivery::getByDeliveryId($id_delivery_label));        
        DeliveryLabel::deliveryLabel($order, $kob_box = $delivery->kob_boxes, $other_box = $delivery->other_boxes, $delivery->delivery_number, $delivery);
    }
}
