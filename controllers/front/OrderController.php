...
class OrderControllerCore extends ParentOrderController
{
        ...
        switch ((int)$this->step) {
            ...
            case OrderController::STEP_ADDRESSES:
                ...
                if (Tools::getValue('multi-shipping') == 1) {
					...
					$countries = $countries->getCountries($this->context->language->id, true);
					$this->context->smarty->assign('countries', $countries);
					
                    if($this->context->cookie->is_perks)
                    {
                        $city = FulfillmentCentre::getCities();
                        $states = FulfillmentCentre::getStates();
                        $this->context->smarty->assign('cities',$city);
                    ...
                    $this->context->smarty->assign('step',1);
            ...
            case OrderController::STEP_PAYMENT:
               ...
               /* if ($is_advanced_payment_api === false && Configuration::get('PS_CONDITIONS')
				...
				$check_details = array(
										'chequeName' => Configuration::get('CHEQUE_NAME'),
										'chequeAddress' => Configuration::get('CHEQUE_ADDRESS'),
										'nbProducts' => $this->context->cart->nbProducts());
				$bank_wire_details = array(
 				...
 				/*All Payment Module Details*/
				 /*$this->context->smarty->assign(array(
					'check_details' => $check_details,
					'bank_wire_details' => $bank_wire_details
					 
				));	*/
				$getCustomerPaymentOption = new DashPaymentOption();
				$result=$getCustomerPaymentOption->getPaymentOptions($this->context->customer->id, true);
				//var_dump($check_details);
 				$this->context->smarty->assign(array(
					'check_details' => $check_details,
                    'step' => 1,
					'bank_wire_details' => $bank_wire_details,
					'available_payment' => $result,
					'id_buyer' => $this->context->customer->id_buyer
					 
				));
				/*$logger = new FileLogger();
				$logger->setFilename('test.txt');
				$logger->logError("Id buyer->".$this->context->customer->id_buyer);*/
    ...
    public function processAddress()
    {
        ...
        if (!Customer::customerHasAddress($this->context->customer->id, (int)Tools::getValue('id_address_delivery'))
			...
			$this->context->cookie->id_state = $address->id_state;
