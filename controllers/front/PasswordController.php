    ...
    public function postProcess()
    {
        if (Tools::isSubmit('email')) {
            ...
            if (!($email = trim(Tools::getValue('email'))) || !Validate::isEmail($email)) {
                ...
                {
                    $this->errors[] = Tools::displayError('Sorry your company is not configured, we will intimate you once it is done.');
                }
                 else {
                    $mail_params = array(
        ...
        } else if (Tools::isSubmit('password') && Tools::isSubmit('confirm_password')) {
                ...
                if (!Validate::isLoadedObject($customer)) {
					...
					if($password === $confirm_password){
						if ($customer->update()) {
                            ...
                            $url = 'https://www.kobster.com/my-account';
                            if($customer->id_buyer == 10)
                                $url = 'https://www.kobster.com/perks';
							$mail_params = array(
								'{email}' => $customer->email,
								'{lastname}' => $customer->lastname,
								'{firstname}' => $customer->firstname,
						...
						$this->errors[] = Tools::displayError('Error in Password creation. Please try again or contact us using contact form');
					}
				}
			}
			else {
            ...
            if ($email) {
                ...
                if (!Validate::isLoadedObject($customer)) {
					...
					$this->context->smarty->assign(array('confirmation' => 1, 'customer_email' => $customer->email, 'token' => $token, 'id_customer' => $id_customer, 'url' => $this->context->link->getPageLink('password') ));
