...
class CategoryControllerCore extends FrontController
{
    ...
    public function setMedia()
    {
        ...
        if (!$this->useMobileTheme()) {
            ...
            $this->addCSS(array(
				...
				_THEME_CSS_DIR_.'allCategory.css'  => 'all'
            ));
        }
		...
		//$this->addJS(_THEME_JS_DIR_.'all-category-chart.js');
    }
    ...
    public function initContent()
    {
		...
		if(CategoryList::isControllerExist($this->category->id))
		{
			
			$category = new CategoryList();
			$category_list_array = $category->getCategoryListDetails($this->category->id);
			
			$subCategory_list_array = $category->getSubCategoryList($this->category->id, 1);
			$subCategory_list_array_nb = $category->getSubCategoryList($this->category->id, 0);

			foreach($subCategory_list_array as $key => $subCategory_list)
			{
				$subCategory[$key]['name']						= $subCategory_list['name'];
				$subCategory[$key]['description'] 				= strip_tags($subCategory_list['description']);
				$subCategory[$key]['subCategory_id']			= $subCategory_list['subCategory_id'];
				$subCategory[$key]['subCategory_image'] 		= $subCategory_list['subCategory_image'];
				$subCategory[$key]['associated_products']		= unserialize($subCategory_list['associated_products']);
				$subCategory[$key]['associated_brands']			= unserialize($subCategory_list['associated_brands']);
			}
			//var_dump($subCategory);
			foreach($subCategory_list_array_nb as $key => $subCategory_list_nb)
			{
				$subCategory_nb[$key]['name']					= $subCategory_list_nb['name'];
				$subCategory_nb[$key]['description'] 			= strip_tags($subCategory_list_nb['description']);
				$subCategory_nb[$key]['subCategory_id']			= $subCategory_list_nb['subCategory_id'];
				$subCategory_nb[$key]['subCategory_image'] 		= $subCategory_list_nb['subCategory_image'];
				$subCategory_nb[$key]['associated_products']	= unserialize($subCategory_list_nb['associated_products']);
			}
			
			foreach($category_list_array as $category_list)
			{
				$category_slider		= unserialize($category_list['list_category_slider']);
				$most_searched 			= unserialize($category_list['list_most_searched']);
				$top_selling 			= unserialize($category_list['list_top_selling']);
				$top_viewed 			= unserialize($category_list['list_top_viewed']);
				$top_discounted 		= unserialize($category_list['list_top_discounted']);
				$kobster_recommented 	= unserialize($category_list['list_kobster_recommented']);
			}
			$msp_list = $category->mostSearchedProducts(implode(',',$most_searched));

			$this->context->smarty->assign(array(
            'category'             	=> $this->category,
            'description_short'    	=> Tools::truncateString($this->category->description, 50),
			...
			'most_searched' 		=> $msp_list,
			'top_selling' 			=> $category->getProducts(1, implode(',',$top_selling)),
			'top_viewed' 			=> $category->getProducts(1, implode(',',$top_viewed)),
			'top_discounted' 		=> $category->getProducts(1, implode(',',$top_discounted)),
			'kobster_recommented' 	=> $category->getProducts(1, implode(',',$kobster_recommented)),
			'subCategory_list'		=> $subCategory,
			'subCategory_list_nb'	=> $subCategory_nb,
            //'return_category_name' => Tools::safeOutput($this->category->name),
            'body_classes'         => array($this->php_self.'-'.$this->category->id, $this->php_self.'-'.$this->category->link_rewrite),
	        ));

			//var_dump($category->getProducts(1, implode(',',$top_selling)));
			
			$this->setTemplate(_PS_THEME_DIR_.'categories.tpl');
