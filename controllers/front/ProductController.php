...
class ProductControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if (!$this->errors) {
            ...
            {
                $perksPrice = Competitor::getPrices($this->product->id)[0];
            }
			
			$this->manufacturer = new Manufacturer((int)$this->product->id_manufacturer, $this->context->language->id);
			$this->manufacturer->description 		= strip_tags($this->manufacturer->description);
			$this->manufacturer->short_description 	= strip_tags($this->manufacturer->short_description);
			
			// Assigning default no image available for brands
            ...
            $this->context->smarty->assign(array(
                ...
                'perksPrice' => $perksPrice,
                'competitorsPrice' => $perksPrice,
                'product_manufacturer' => $this->manufacturer,
    ...
    protected function assignAttributesGroups()
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach ($attributes_groups as $k => $row) {
                ...
                $groups[$row['id_attribute_group']]['p_attributes'][$row['id_attribute']] = $row['id_product_attribute'];
