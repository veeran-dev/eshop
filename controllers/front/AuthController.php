...
class AuthControllerCore extends FrontController
{
    ...
    public function initContent()
    {
		...
		$state = new State();
		$state_names = $state->getStatesByIdCountry((int)$this->id_country);
        ...
        if($this->context->cookie->is_perks)
        ...
        if (empty($email)) {
            ...
            if (isset($authentication->active) && !$authentication->active) {
                ...
                if (!$this->ajax) {
                    ...
                    if($customer->id_buyer == 10) {
                        Tools::redirect('index.php?controller=perksDeals');
                    }
					else if($customer->id_buyer !=3){
    ...
    protected function processSubmitAccount()
    {
        ...
        $generateVoucher=$_POST['generateVoucher'];
        $this->context->cookie->error=0;
        ...
        if (Validate::isEmail($email = Tools::getValue('email')) && !empty($email)) {
            if (Customer::customerExists($email)) {
                if($generateVoucher)
                {
                    ...
                    $this->context->cookie->error="1";
                    $url=Tools::getValue('currentUrl');
                    header("Location:".$url."");
                }
                else
		...
		$customer->id_buyer = $_POST['id_buyer'];

		$google_captcha_response = Tools::getValue('g-recaptcha-response');
		
        if(!$customer->verifyIsHuman($google_captcha_response)) {
            $this->errors[] = Tools::displayError('Please verify that you are not a robot by clicking "I am not a robot.');
        }
        ...
        {
            $customer->mobile=$_POST['phone_mobile'];
        }
            ...
            if (!count($this->errors)) {
                ...
                if (!count($this->errors)) {
                    ...
                    if ($customer->add()) {
                        ...
                        if($generateVoucher)
                        {
                            $_POST['pincode']=$_POST['postcode'];
                            $_POST['type']=2;
                            $this->generateVoucher($customer->id);
                            $this->setStates();
                        }
                        

                        // redirection: if cart is not empty : redirection to the cart
                        if ((count($this->context->cart->getProducts(true)) > 0) && !$generateVoucher){
                            $multi = (int)Tools::getValue('multi-shipping');
                            Tools::redirect('index.php?controller=order'.($multi ? '&multi-shipping='.$multi : ''));
                        }
                        // else : redirection to the account
                        elseif(!$generateVoucher) {
                            Tools::redirect('index.php?controller='.(($this->authRedirection !== false) ? urlencode($this->authRedirection) : 'my-account'));
                        }
                        else
                        {
        ...
        if (!count($this->errors)) {
			...
			/*Check for customer verification document*/
			if($file != "" )
				{
            ...
            if (!count($this->errors)) {
                ...
                if (!$customer->add()) {
					...
					if($file !="")
					{
						/*Adding customer verification document*/
    ...
    public function generateVoucher($id_customer)
    {
        ...
        $this->context->cookie->voucherCode = $cart_rule->code;
        $this->context->cookie->write();


        /***Voucher Mail***/
        Mail::Send($this->context->language->id,'voucher-for-deals',Mail::l('Kobster voucher code'),array('{firstname}' => $customer->firstname,'{voucher_num}' => $cart_rule->code),$customer->email,$customer->firstname);
    }
