...
class ParentOrderControllerCore extends FrontController
{
                    ...
                    if (!($code = trim(Tools::getValue('discount_name')))) {
                        ...
                        if (($cartRule = new CartRule(CartRule::getIdByCode($code))) && Validate::isLoadedObject($cartRule)) {
                            if ($error = $cartRule->checkValidity($this->context, false, true)) {
								...
								{
									$back = explode("?",Tools::getValue('back'));
                                    if($back[1] == ""){
                                        Tools::redirect('index.php?controller=order');
                                    }
                                    else{
                                        Tools::redirect('index.php?'.$back[1]);
                                    }
    ...
    protected function _assignSummaryInformations()
    {
        ...
        if($this->context->cookie->voucherCode)
        {
            $cart_rule = new CartRule($this->context->cookie->voucherCode);
            if($cart_rule->quantity == 1)
            {
                $perksVoucherCode = $cart_rule->code;
            }
        }
        ...
        $this->context->smarty->assign(array(
            ...
            'perksVoucherCode' => $perksVoucherCode,
