...
class AddressControllerCore extends FrontController
{
    ...
    protected function processSubmitAddress()
    {
		...
		if (Tools::getValue('phone')) {
			$phone_len = strlen(Tools::getValue('phone'));
			if($phone_len != '10'){
				$this->errors[] = Tools::displayError('Please enter Valid 10 digit Phone number.');
			}
        }
		if (Tools::getValue('phone_mobile')) {
			$phone_mobile_len = strlen(Tools::getValue('phone_mobile'));
			if($phone_mobile_len != '10'){
				$this->errors[] = Tools::displayError('Please enter Valid 10 digit Mobile number.');
			}
        }
        if (Tools::getValue('city')) {
            $city = Tools::getValue('city');
            if($city == '' || $city == 'Select City'){
                $this->errors[] = Tools::displayError('Please Select valid city');
            }
        }
        ...
        if ($result = $address->save()) {
			...
			$id_address=Address::getAddressId((int)$this->context->cookie->id_customer);
			CustomerAddress::mapId($this->context->cookie->id_customer,$id_address[0]['id_address']);
