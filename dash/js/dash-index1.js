...
$(document).ready(function() {
	...
	loadVoucherCode(0);
	$("#dash_user_search_query_top").val('');
	$('#dash_user_search_query_top_button').click(function()
	{
		var queryValue=$('#dash_user_search_query_top').val();
		if(queryValue=="")
		{
			return false;
             ...
             if (data) 
			 {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (data[i] == "" || data == "null") {
                        ...
                        $('#order-status-in-process').append("<div class='order-status-set-height'><div id='order-status-in-process" + i + "' class='col-md-9'></div><div class='col-md-3 padding-align-left estimate-time' id='order-state-estimate-time" + i + "'></div><div id='order-status-in-process-text" + i + "' class='col-md-9 '><span class='col-md-3'></span></div></div>");
                        ...
                        $('#order-status-in-process' + i).append("<span  class='col-md-3 track-order-id-value'  id='order-id-in-process" + i + "'><a data-toggle='modal' href='#status_history' onclick='getStatusDetails(" + data[i][0].id_order + "," + data[i][0][0] + "," + data[i][0][1] + ")'>" + data[i][0].id_order + "</a></span>");
...
function draw() {
    setTimeout(function() {
        $(".animate-span").css("width", "47px");
    }, 500);
    setTimeout(function() {
        $(".order-status-name").css("display", "block");
    }, 2500);
}
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; i < data[3].length; i++) {
                $("#productsTable").append("<tr><td>"+(i+1)+"</td><td>" + data[3][i].product_reference + "</td><td>" + data[3][i].product_name + "</td><td>" + data[3][i].product_quantity + "</td><td>" + data[3][i].product_price_wt + "</td><td class='price'>" + (data[3][i].total_wt).toFixed(2) + "</td></tr> ");
            }
            ...
            if (data[5].total_paid_real != 0 && data[5].total_paid_real != "") {
                if (data[4] == 1) {
                ...
                $("#productsTable").append("<tr><td></td> <td></td><td></td><td></td><td><strong>" + discount_price_display + "</strong></td><td class='price'> <span ><strong> " + (data[5].total_discounts).toFixed(2) + " </strong></span></td></tr><tr><td></td> <td></td><td></td><td></td> <td><strong>" + taxprice + "</strong></td><td class='price'> <span ><strong> " + (data[5].total_paid_real).toFixed(2) + " </strong></span></td></tr>");
...
function submitMyAddress()//to get the new user address
	{
		...
		{
			$('#add_state').val('');
			$('#add_state').attr("placeholder",'please Select State');
 			error=1;
 		}
   	...
   	$.ajax({
 			...
 			success: function(data)
			{
				...
				/*loyalty-points section starts here*/
					...
					$("#customer_points").append("Loyalty Points: &nbsp;<span class='normal_font_weight'>"+data[2]+"</span>");
				...
				{	
				 ...
				 //alert(rowCount);
					...
					$('#show_voucher_count').append("Vouchers: <span class='normal_font_weight'>"+data[0].length+"</span>");
 ...
 function quickView(id,idList)
{
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#id_quick").html("<a class='quickViewClose' onclick='closeQuickView();'><button class='close' aria-hidden='true' data-dismiss='modal' type='button'>×</button></a><div class='quickviewpurchaselistleft'><div class='quickViewproductimglink'><img src="+data[0]+"></div><div class='quickviewpurchaselistdesc'>"+str+"</div></div><div class='quickviewpurchaselistright' ><h3 class='quickviewheader'>"+data.name+"</h3><div id='quickviewsec'><div id='Manufactured-id'><span>Manufactured by:</span><span class='data'>"+data[1]+"</span></div><br><br><div class='product_code'><span>Product Code:</span><span class='data'>"+data.reference+"</span></div><br></div><div id='price_section'><div class='quick_price'><span class='span_float_left' >"+data[3]+"</span></span><span id='quick-label' class='text-muted search-nolist'></span></div></div><div id='Addtolist' ></div><div class='quickView_carousel_buttons'><div id='quick_quantity' ><div id='quick_quantity_div'>QTY: </div><input type='text' onkeydown='numbersOnly(event)' class='quickView_quantity_wanted' maxlength='5' size='4' value='"+val+"'><label id='error_qty'></label><label class='txt-bottom '>Minimum quantity:"+data['minimal_quantity']+"</label></div></div><div id='quick_button' onclick='addToCart(0,"+data.id_product+")'><span class='fa fa-shopping-cart quick_cart'></span> <label id='quick_buynow'>Buy Now</label></div><div id='quick_call2buy'>Contact your Relationship Manager for any queries</div></div> <div id='quick_bottom'><div id='quick_gen'><span id='static_100' style='color:#7f7f7f;'>100% GENUINE & BRAND NEW PRODUCTS</span></div><div id='quick_ship'><span='static_ship' style='color:#7f7f7f;'>FREE SHIPPING FOR ALL CORPORATE CUSTOMER</span></div></div> </div>");
...
function addpurlist(idProduct){
    ...
    var dataparam = '&id_product='+idProduct+'&id_customer='+id_customer+'&id_list='+id;
    $.ajax({
        type: 'POST',
        async: true,
