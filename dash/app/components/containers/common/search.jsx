...
class Search extends React.Component { 
	...
	handleValuesChange(component, values)
	{
		...
		document.getElementById('priceMin').value=this.state.values.min;
		document.getElementById('priceMax').value=this.state.values.max;
	...
	handleValuesChangeDone()
	{
		...
		procureBuyApi.getFilterValues(this.state.limit,this.state.idPage,this.state.search_type,this.state.pageType,this.props.params.searchTerm);
	}
    componentDidMount(){
	...
	filterProducts(type, limit,idPage, updatePagination,freshclick,page) {
 	...
 	checkFilter(elemId,limit,idPage,search_type,pagenation, dynamicObject) {
		...
		if(dynamicObject != undefined) {
 		...
 		procureBuyApi.getFilterValues(limit,idPage,search_type,this.state.pageType,this.props.params.searchTerm);
		this.setState({
			limit: limit,
			idPage: idPage,
					...
					<form id="categoryFilterForm">
						{categoryFilter != undefined && categoryFilter ? 
							categoryFilter.map((data, i) => {
								return (
									<div key={i}>
										<Input id={data.id_category} type="checkbox" name={"cat_"+data.id_category} defaultChecked={false} />
										<label for={data.id_category} onClick={this.checkFilter.bind(this, data.id_category,this.state.limit,this.state.idPage,this.state.search_type,false)}>{data.name}</label>
								    </div>
								)
							}) :
							null}
					...
					<form id="manufacturerFilterForm">
						{manufactureFilter != undefined && manufactureFilter ? 
							manufactureFilter.map((data, i) => {
								return (
									<div key={i}>
										<Input id={data.id_manufacturer} type="checkbox" name={"brand_"+data.id_manufacturer} defaultChecked={false} />
										<label for={data.id_manufacturer} onClick={this.checkFilter.bind(this, data.id_manufacturer,this.state.limit,this.state.idPage,this.state.search_type,false)}>{data.name}</label>
								    </div>
								)
							}) :
							null}
						...
						<div className="price-slider">
						  ...
						  <div className="input-range-slider">
							<InputRange
							 ...
							 // onMouseUp={this.handleValuesChange.bind(this)}
                  ...
                  <div key={index} className="product">                    
                    ...
                    <a href="javascript:void(0)" onClick={G.quickView.bind(this, products.id_product)}>
                      <div className="product-visual">
                        ...
                        <button className="quick-view" >Quik view</button>
                      </div>
                    </a>
                    ...
                    <div className="product-actions">
                      ...
                      <div className="product-cta"><button className="button-black" type="button" onClick={G.addToCart.bind(this, products.id_product, "search_results_quantity", 0, products.minimal_quantity)}>Add to Cart</button>
                      </div>
                    </div>
