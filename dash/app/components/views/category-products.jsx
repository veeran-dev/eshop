...
class CategoryProducts extends React.Component {
	...
	handleValuesChangeDone() {
		procureBuyApi.getFilterValues(this.state.limit,this.state.idPage,this.state.search_type,this.state.pageType,this.props.params.categoryId, this.state.values);
	}
	
    componentDidMount() {
    	/*G.getCategoryById(this.props.params.categoryId,this.state.limit,this.state.idPage,this.state.search_type)
  	    G.getCategoryNameById(this.props.params.categoryId)*/
  	    G.getCategoryNameById(this.props.params.categoryId)
		G.getListCategoryProductsWithFilters(this.props.params.categoryId,this.state.limit,this.state.idPage,this.state.search_type)
    }
	
	componentWillReceiveProps(newProps)
	{
 		if(this.props.params.categoryId != newProps.params.categoryId)
		{
			//G.getCategoryById(newProps.params.categoryId, this.state.limit, this.state.idPage, this.state.search_type)
		  G.getCategoryNameById(newProps.params.categoryId)
		  G.getListCategoryProductsWithFilters(newProps.params.categoryId,this.state.limit,this.state.idPage,this.state.search_type)
		}
	}
	...
	filterProducts(type, limit,idPage, updatePagination,freshclick,page ) {
      ...
      let page_id = (page && page.selected) ? page.selected : id_Page;
       G.getListCategoryProductsWithFilters(this.props.params.categoryId, limit, page_id,type)
      this.setState({
        limit: limit,
        idPage: page_id,
        search_type: type,
	...
	checkFilter(elemId,limit,idPage,search_type,pagenation, dynamicObject) {
		...
		if(dynamicObject != undefined) {
 		...
 		procureBuyApi.getFilterValues(limit,idPage,search_type,this.state.pageType,this.props.params.categoryId, this.state.values);
		this.setState({
			limit: limit,
			idPage: idPage,
  ...
  render() {
	 ...
	 if(priceFilter != undefined) {
		   var priceMin,priceMax;
 				...
 				priceFilterResult.push(<div key={0}>
        ...
        {/* Filters block Ends here*/}
           <div className="page-header">
            <h3 className="page-title">Showing results for <span id="categorySearchKeyword" className="red-color-text"></span></h3>
            ...
            <div className="pagination-container">
              <ReactPaginate 
                    ...
                    pageNum={this.props.productList && this.props.productList.length > 0 ? this.props.totalCount : 0}
          ...
          <ReactCSSTransitionGroup transitionName="grid-animation" transitionAppear={true} transitionAppearTimeout={500} transitionLeaveTimeout={300} transitionEnterTimeout={300} component="div" className="product-container">
            {this.props.productList && this.props.productList.length > 0 ? this.props.productList.map((products, index) => {
...
const mapStateToProps = function(store) {
  ...
  return {
  	...
  	productList: store.procureBuyState.productList,
